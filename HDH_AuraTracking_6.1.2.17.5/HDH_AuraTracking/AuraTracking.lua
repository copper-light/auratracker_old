...
--------------------------------------------
	...
	local function GetAuras(self, filter, isbuff)
		...
		for i = 1, 40 do 
			name, _, _, count, dispelType, duration, endTime, caster, _, _, id, _, _, v1, v2, v3 = UnitAura(self.unit, i, filter)
	...
	local function GetAurasAll(self, filter, isbuff, startIdx) 
		local curTime = GetTime()
		local ret = 1
		for i = 1, 40 do 
			...
			if not id then ret = (startIdx or 0) + i break end
			f = self.frame.icon[(startIdx or 0) + i]
			if f then
				if not f.spell then
					f.spell = {}
				end
	...
	function HDH_TRACKER:Update()
		...
		if self.option.base.check_buff then 
			ret = self.GetAurasFunc(self, "HELPFUL", true)
		end
		
		if self.option.base.check_debuff then 
			self.GetAurasFunc(self, "HARMFUL"..(mine or ""), false, (self.option.base.tracking_all and ret or nil))
		end
