...
local DDM_FONT_LIST = {"시간 글자", "중첩 글자", "1차 수치 글자", "2차 수치 글자"}
local SET_KEY_FONT = {}
...
function HDH_LoadListFrame(tabIdx, startRowIdx, endRowIdx)
	...
	local aura = {}
	local tracker_name = GetTabUnitInfo(tabIdx or 1)
	if not DB_AURA.Talent[CurSpec] then return end
	aura = DB_AURA.Talent[CurSpec][tracker_name]
	local rowFrame
	local i = startRowIdx or 1
	
	if DB_OPTION[tracker_name] and DB_OPTION[tracker_name].tracking_all then
		HDH_AT_NoList:Show()
	else
...
function ShowAddFrame(bool, isModify)
	if isModify then
		if bool then
			...
			if HDH_IS_UNIT[unit] then
				...
				AddUnitTabFrameCheckButtonDefaultColor:SetChecked(DB_OPTION[name].defaultColor)
				AddUnitTabFrameCheckButtonDefaultColor:Show()
				...
				AddUnitTabFrameCheckButtonDefaultColor:Hide()
		...
		if bool then
			...
			AddUnitTabFrameCheckButtonDefaultColor:SetChecked(false)
			AddUnitTabFrameCheckButtonDefaultColor:Show()
			AddUnitTabFrameCheckButtonBuff:Show()
			AddUnitTabFrameCheckButtonDebuff:Show()
			AddUnitTabFrameCheckButtonMine:Show()
...
function HDH_TabUnit_ChangeTab(self, idx)
	...
	if UnitOptionFrame.AddBtn then
		ShowAddFrame(UnitOptionFrame.AddBtn == self or idx == 0 )
		SetActiveUnitTab(UnitOptionFrame.AddBtn, UnitOptionFrame.AddBtn == self or idx == 0)
	end
	...
	if name ~= "" and unit then
		if mode == "add" then
			...
			if not tracker then
				...
				DB_OPTION[name].defaultColor = AddUnitTabFrameCheckButtonDefaultColor:GetChecked()	
...
function HDH_LoadUnitUI(tabIdx)
	...
	_G["UnitOptionFrameCheckButtonTooltip"]:SetChecked(DB_OPTION[name].show_spell_tooltip)
...
function HDH_SetModeUI(mode)
	...
	if mode == MODE_EDIT then
		...
		UnitOptionFrameCheckButtonTooltip:Hide()
	...
	elseif mode == MODE_SET_ALL then
		...
		UnitOptionFrameCheckButtonTooltip:Show()
...
function HDH_SetModeSettingFrame(mode)
	...
	local profile = _G[SettingFrame:GetName().."ButtonProfile"]
	...
	if mode == MODE_SET_EACH then
		...
		SettingFrame:SetPoint("BOTTOMRIGHT",SettingFrame:GetParent(),"BOTTOMRIGHT",-20, 60);
		close:Hide()
		reset_ui:Show()
		reset_aura:Show()
		id_show:Show()
...
function HDH_OnShowSpellTooltip(check)
	local name = GetTabUnitInfo(CurTab)
	if not name then return end
	DB_OPTION[name].show_spell_tooltip = check
	if not UI_LOCK then
		local t = HDH_TRACKER.Get(name)
		if t then t:UpdateSetting() end
	end
...
function HDH_OnCheckedDefaultColor(checked)
	local name = GetTabUnitInfo(CurTab)
	local tracker = HDH_TRACKER.Get(name)
	if tracker then
		DB_OPTION[name].defaultColor = checked
		if UI_LOCK then tracker:SetMove(UI_LOCK)
				   else tracker:InitIcons() end
	end
end
...
function HDH_LoadChangeIconFrame(self, row, spec, tab) ---------------------------------------------------
	...
	local data = DB_AURA.Talent[self.spec][GetTabUnitInfo(self.tab)][row]
...
function HDH_OnClickChangeIcon(self)
	...
	if parent.texture then 
		...
		_G[RowDetailSetFrame:GetName().."Icon"]:SetTexture(parent.texture)
	else
		parent.texture = nil
		print("|cffff0000AuraTracking:Error - |cffffff00알 수 없는 이름(ID) 입니다")
	end
...
function HDH_OnClickRestoreIcon(self)
	local parent = self:GetParent()
	local data = DB_AURA.Talent[parent.spec][GetTabUnitInfo(parent.tab)][parent.row]
	if not data then return end
	if data.IsItem then
...
function HDH_LoadGlowFrame(self, row, spec, tab)--------------------------------------------------------
	...
	if db[tonumber(row)].Glow then _G[self:GetName().."Disable"]:Hide()
							  else _G[self:GetName().."Disable"]:Show() end
...
function HDH_OnGlow(self, no, check)
	local parent = self:GetParent()
	local name = GetTabUnitInfo(parent.tab)
	if not name then return end
	local db = DB_AURA.Talent[parent.spec][name]
	if not db[tonumber(parent.row)] then return end
	if check then
		_G[parent:GetName().."Disable"]:Hide()
	else
		_G[parent:GetName().."Disable"]:Show()
	end
	db[tonumber(parent.row)].Glow = check
	local t = HDH_TRACKER.Get(name)
	if t then t:InitIcons() end
end
...
function HDH_LoadValueFrame(self, row, spec, tab) -------------------------------------------------------
	local name, unit = GetTabUnitInfo(tab)
	if not name then return end
	local db = DB_AURA.Talent[spec][name]
	if not db[tonumber(row)] then return end
	db = db[tonumber(row)]
	...
	if HDH_IS_UNIT[unit] then
		_G[self:GetName().."Info"]:Hide()
		_G[self:GetName().."Info2"]:Show()
		_G[self:GetName().."Disable"]:ClearAllPoints()
		_G[self:GetName().."Disable"]:SetPoint("TOPLEFT",3,-30)
		_G[self:GetName().."Disable"]:SetPoint("BOTTOMRIGHT",-3,3)
	else
		_G[self:GetName().."Info"]:Show()
		_G[self:GetName().."Info2"]:Hide()
		_G[self:GetName().."Disable"]:ClearAllPoints()
		_G[self:GetName().."Disable"]:SetPoint("TOPLEFT",3,-3)
		_G[self:GetName().."Disable"]:SetPoint("BOTTOMRIGHT",-3,3)
	end
	
	if db.ShowValue then
		_G[self:GetName().."Disable"]:Hide()
	else
		_G[self:GetName().."Disable"]:Show()
	end
...
function HDH_OnShowValue(self, no, check)
	...
	db[tonumber(parent.row)].ShowValue = check
	local t = HDH_TRACKER.Get(name)
	if t then t:InitIcons() end
end
...
function HDH_LoadSoundFrame(self, row , spec, tab) ------------------------------------------------------
	...
	local db = DB_AURA.Talent[spec][GetTabUnitInfo(tab)]
	if not db[tonumber(row)] then return end
	db = db[tonumber(row)]
...
function HDH_OnClickSaveSound(self)
	...
	local db = DB_AURA.Talent[parent.spec][GetTabUnitInfo(parent.tab)]
	if not db[tonumber(parent.row)] then return end
	...
	local t = HDH_TRACKER.Get(GetTabUnitInfo(parent.tab))
	if t then t:InitIcons() end
end
...
function HDH_OnClick_RowDetailSet(self, row) ---------------------------------------------
	...
	local name = _G[frame:GetName().."Text"]
	local icon = _G[frame:GetName().."Icon"]
	name:SetText(_G[self:GetParent():GetName().."TextName"]:GetText())
	icon:SetTexture(db.Texture)
	RowDetailSetFrame:Show()
	
	HDH_LoadSoundFrame(SoundFrame, row , spec, tab)
	HDH_LoadChangeIconFrame(ChangeIconFrame, row, spec, tab)
	HDH_LoadValueFrame(ValueOptionFrame, row, spec, tab)
	HDH_LoadGlowFrame(GlowOptionFrame, row, spec, tab)
end
...
function HDH_Option_OnShow(self)
	...
	if CurTab == 0 then
		HDH_TabUnit_ChangeTab(UnitOptionFrame.AddBtn)
	else
		HDH_LoadTabSpec()
		HDH_TabUnit_Load()
		HDH_RefrashSettingUI(MODE_SET_ALL)
...
function HDH_Option_OnLoad(self)
	...
	self:SetMinResize(FRAME_W, 410) 
	self:SetMaxResize(FRAME_W, 900)
