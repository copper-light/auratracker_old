...
do 
	...
	function HDH_C_TRACKER:Update() -- HDH_TRACKER override
		if UI_LOCK or not self.option or not self.option.base then return end
		if self.option.base.relative_unit and not UnitExists(self.option.base.relative_unit) then
	...
	function HDH_C_TRACKER:InitIcons() -- HDH_TRACKER override
		...
		self.frame:SetScript("OnEvent", CT_OnEvent_UnitChanged)
		self.frame:UnregisterAllEvents()
		self.frame:RegisterEvent('PLAYER_TALENT_UPDATE')
		if #(self.frame.icon) > 0 and self.option.base.relative_unit then
			if self.option.base.relative_unit == 'target' then
				self.frame:RegisterEvent('PLAYER_TARGET_CHANGED')
			elseif self.option.base.relative_unit == 'focus' then
				self.frame:RegisterEvent('PLAYER_FOCUS_CHANGED')
			elseif string.find(self.option.base.relative_unit, "boss") then 
				self.frame:RegisterEvent('INSTANCE_ENCOUNTER_ENGAGE_UNIT')
			elseif string.find(self.option.base.relative_unit, "party") then
				self.frame:RegisterEvent('GROUP_ROSTER_UPDATE')
			elseif self.option.base.relative_unit == 'pet' then
	...
	function HDH_C_TRACKER:ACTIVE_TALENT_GROUP_CHANGED()
		self:RunTimer("PLAYER_TALENT_UPDATE", 0.2, self.InitIcons, self.name, self.unit)
	end
...
function CT_SPELL_ACTIVATION_OVERLAY_GLOW_SHOW(f, id)
	if f and f.spell and f.spell.id == id then
		HDH_C_TRACKER:ActionButton_ShowOverlayGlow(f)
	end
end
...
function CT_OnEvent_UnitChanged(self, event, ...)
	...
	elseif event == 'PLAYER_TALENT_UPDATE' then
		tracker:RunTimer("PLAYER_TALENT_UPDATE", 0.2, tracker.InitIcons, tracker.name, tracker.unit)
	end
end
