...
-- local POWRE_NAME = {}
local L_POWRE_MANA = "자원:마나";
local L_POWRE_RAGE = "자원:분노"
local L_POWRE_FOCUS = "자원:집중"
local L_POWRE_ENERGY = "자원:기력"
local L_POWRE_RUNIC_POWER = "자원:룬마력"
local L_POWRE_LUNAR_POWER = "자원:천공의힘"
local L_POWRE_MAELSTROM = "자원:소용돌이"
local L_POWRE_INSANITY = "자원:광기"
local L_POWRE_FURY = "자원:격노"
local L_POWRE_PAIN = "자원:고통"

local HDH_POWER = {}
HDH_POWER[L_POWRE_MANA]		 	= {power_type="MANA",		 	power_index =0,		color={0.25, 0.78, 0.92}, 		texture = "Interface\\Icons\\INV_Misc_Rune_03"};
HDH_POWER[L_POWRE_RAGE]			= {power_type="RAGE", 			power_index =1,		color={0.77, 0.12, 0.23}, 		texture = "Interface\\Icons\\Ability_Warrior_Rampage"};
HDH_POWER[L_POWRE_FOCUS] 		= {power_type="FOCUS", 			power_index =2,		color={1.00, 0.50, 0.25}, 		texture = "Interface\\Icons\\Ability_Fixated_State_Orange"};
HDH_POWER[L_POWRE_ENERGY]		= {power_type="ENERGY",			power_index =3, 	color={1, 0.96, 0.41}, 	  		texture = "Interface\\Icons\\Spell_Holy_PowerInfusion"};
HDH_POWER[L_POWRE_RUNIC_POWER]	= {power_type="RUNIC_POWER", 	power_index =6,		color={0, 0.82, 1}, 			texture = "Interface\\Icons\\Spell_DeathKnight_FrozenRuneWeapon"};
HDH_POWER[L_POWRE_LUNAR_POWER] 	= {power_type="LUNAR_POWER",	power_index =8, 	color={0.30, 0.52, 0.90},		texture = "Interface\\Icons\\Ability_Druid_Eclipse"};
HDH_POWER[L_POWRE_MAELSTROM]	= {power_type="MAELSTROM", 		power_index =11,	color={0.25, 0.5, 1},			texture = "Interface\\Icons\\Spell_Shaman_StaticShock"};
HDH_POWER[L_POWRE_INSANITY] 	= {power_type="INSANITY", 		power_index =13,	color={0.70, 0.4, 0.90},	  	texture = "Interface\\Icons\\SPELL_SHADOW_TWISTEDFAITH"};
HDH_POWER[L_POWRE_FURY] 		= {power_type="FURY",			power_index =17, 	color={0.788, 0.259, 0.992},	texture = "Interface\\Icons\\Spell_Shadow_SummonVoidWalker"};-- 17
HDH_POWER[L_POWRE_PAIN] 		= {power_type="PAIN",			power_index =18,	color={1, 156/255, 0}, 			texture = "Interface\\Icons\\Ability_Warlock_FireandBrimstone"}; -- 18
...
if MyClass == "MAGE" or MyClass == "PALADIN" or MyClass == "WARLOCK" then 
	TrackerTypeName[1] = L_POWRE_MANA;
elseif MyClass == "PRIEST"  then
	TrackerTypeName[1] = L_POWRE_MANA;
	TrackerTypeName[2] = L_POWRE_INSANITY;
elseif MyClass == "WARRIOR" then 
	TrackerTypeName[1] = L_POWRE_RAGE
elseif MyClass == "DRUID" then 
	TrackerTypeName[1] = L_POWRE_MANA
	TrackerTypeName[2] = L_POWRE_LUNAR_POWER
	TrackerTypeName[3] = L_POWRE_ENERGY
	TrackerTypeName[4] = L_POWRE_RAGE
elseif MyClass == "DEATHKNIGHT" then TrackerTypeName[1] = L_POWRE_RUNIC_POWER;
elseif MyClass == "HUNTER" then TrackerTypeName[1] = L_POWRE_FOCUS; 
elseif MyClass == "ROGUE" then TrackerTypeName[1] = L_POWRE_ENERGY 
elseif MyClass == "SHAMAN" then 
	TrackerTypeName[1] = L_POWRE_MANA
	TrackerTypeName[2] = L_POWRE_MAELSTROM
elseif MyClass == "MONK" then TrackerTypeName[1] = L_POWRE_ENERGY
elseif MyClass == "DEMONHUNTER" then 
	TrackerTypeName[1] = L_POWRE_FURY; 
	TrackerTypeName[2] = L_POWRE_PAIN;
end
...
for i = 1, #TrackerTypeName do
	...
	HDH_POWER_TRACKER[TrackerTypeName[i]] = true;
end
...
------------------------------------
	...
	local function HDH_POWER_OnUpdate(self)
		...
		if self.spell.curTime - (self.spell.delay or 0) < 0.03  then return end 
		...
		if self.spell.showPer then
			self.spell.count = (self.spell.v1/UnitPowerMax('player', self.spell.power_index) * 100);
			self.counttext:SetText(format("%d%%", self.spell.count)); 
		else self.counttext:SetText(nil) end
		
		if self.spell.showValue then self.v1:SetText(HDH_AT_UTIL.CommaValue(self.spell.v1)); else self.v1:SetText(nil) end
	...
	-- local function GetAnimatedValue(bar, v,i)
	...
	function HDH_POWER_TRACKER:UpdateBarValue(f)
		if f.bar and #f.bar.bar > 0 then
			...
			for i = 1, #f.bar.bar do 
				...
				if bar then 
					-- bar:SetValue(GetAnimatedValue(bar,f.spell.v1,i)); 
					...
					if self.option.bar.show_spark then
						if f.spell.v1 >= bar.mpMax then value = 1; if bar.spark:IsShown() then bar.spark:Hide(); end
						elseif f.spell.v1<= bar.mpMin then value = 0; if bar.spark:IsShown() then bar.spark:Hide(); end
						else
							value = (f.spell.v1-bar.mpMin)/(bar.mpMax - bar.mpMin);
	...
	-- function HDH_POWER_TRACKER:UpdateTalentInfo()
		...
		-- HDH_POWER_TRACKER.HDH_PT_KEY = HDH_POWER_TRACKER.SPEC_INFO[id].TYPE;
		-- HDH_POWER_TRACKER.HDH_POWER_INDEX = HDH_POWER_TRACKER.SPEC_INFO[id].INDEX;
		-- HDH_POWER_TRACKER.HDH_POWER_NAME = HDH_POWER_TRACKER.SPEC_INFO[id].TYPE;
		-- HDH_POWER_TRACKER.SPEC_INFO[id].TEXTURE = POWER_TEXTURE[HDH_POWER_TRACKER.HDH_POWER_NAME];
		-- HDH_POWER_TRACKER.HDH_POWER_TEXTURE = HDH_POWER_TRACKER.SPEC_INFO[id].TEXTURE;
		
		-- HDH_POWER_TRACKER.HDH_POWER_TYPE = {};
		-- HDH_POWER_TRACKER.HDH_POWER_TYPE[HDH_POWER_TRACKER.HDH_POWER_NAME] = HDH_POWER_TRACKER.HDH_POWER_INDEX;
	-- end
	function HDH_POWER_TRACKER:CreateData(spec)
		if spec and DB_AURA.Talent[spec] then
			local new = {}		
			new.Key = HDH_POWER[self.unit].power_type..1;
			new.Name = HDH_POWER[self.unit].power_type..1;
			new.Texture = HDH_POWER[self.unit].texture;
			...
			DB_OPTION[self.name].bar.color = {unpack(HDH_POWER[self.unit].color)};
			DB_OPTION[self.name].bar.full_color = {unpack(HDH_POWER[self.unit].color)};
	...
	function HDH_POWER_TRACKER:IsHaveData(spec)
		if spec and DB_AURA.Talent[spec] then
			...
			if data and string.find(data.Key, HDH_POWER[self.unit].power_type) then
				return 1;
			end
		end
		return false;
	...
	function HDH_POWER_TRACKER:CreateDummySpell(count)
		...
		local f, spell
		local power_max = UnitPowerMax("player",HDH_POWER[self.unit].power_index);
		f = icons[1];
	...
	function HDH_POWER_TRACKER:UpdateBar(f)
		...
		local isAfterNill = false;
		bf.max = UnitPowerMax("player",HDH_POWER[self.unit].power_index);
		-- 무결성 체크
		for i = 1, #split do
			if isAfterNill then 
				split[i] = nil 
	...
	-- function HDH_POWER_TRACKER:CreateData(cnt)
		-- local talent = DB_AURA.Talent[GetSpecialization()] 
		-- if not talent then return end 	
		-- talent[self.name] = {}
		-- local auraList = talent[self.name]
		-- local new = {}
		-- new.Key = HDH_POWER_TRACKER.HDH_PT_KEY
		-- new.Name = HDH_POWER_TRACKER.HDH_POWER_NAME
		-- new.Texture = HDH_POWER_TRACKER.HDH_POWER_TEXTURE
		-- new.defaultImg = HDH_POWER_TRACKER.HDH_POWER_TEXTURE
		-- new.ShowValue = true;
		-- new.No = 1
		-- new.ID = 0
		-- new.Always = true
	...
	-- function HDH_POWER_TRACKER:CreateUIData()
		-- self:UpdateSetting();
	-- end
	...
	function HDH_POWER_TRACKER:InitIcons() -- HDH_TRACKER override
		...
		if self:IsHaveData(self:GetSpec()) then
			if string.find(auraList[1].Key or "", HDH_POWER[self.unit].power_type)  then
				ret = 1
				f = self.frame.icon[1]
				...
				spell.happenTime = 0;
				spell.isItem = false;
				if HDH_POWER[self.unit].power_type == "MANA" then
					spell.showPer = true;
				end
	...
	function HDH_POWER_TRACKER:OnEvent(event, unit, powerType)
		...
		if ((event == 'UNIT_MAXPOWER')) and (HDH_POWER[self.parent.unit].power_type == powerType) then  -- (event == "UNIT_POWER")
			if not UI_LOCK then
				...
				self:UpdateBar(self.parent.frame.icon[1]);
			end
		end
	end
------------------------------------
