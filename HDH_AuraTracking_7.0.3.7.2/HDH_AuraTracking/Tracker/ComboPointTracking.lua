...
if TrackerTypeName then
	...
	HDH_COMBO_POINT_TRACKER[TrackerTypeName] = true;
end
...
------------------------------------
	...
	function HDH_COMBO_POINT_TRACKER:UpdateIcons()  -- HDH_TRACKER override
		...
		for k,f in ipairs(icons) do
			...
			if f.spell.isUpdate then
				...
				if k <= UnitPowerMax('player', HDH_POWER[self.unit].power_index) then 
	...
	function HDH_COMBO_POINT_TRACKER:Update() -- HDH_TRACKER override
		...
		local power = UnitPower('player', HDH_POWER[self.unit].power_index, true);
		local power_max = UnitPowerMax('player', HDH_POWER[self.unit].power_index);
		...
		self:UpdateIcons();
		if self.option.icon.always_show or UnitAffectingCombat("player") or ret >= 1 then
			self:ShowTracker();
		else
			self:HideTracker();
		end
	...
	function HDH_COMBO_POINT_TRACKER:InitIcons() -- HDH_TRACKER override
		...
		local f
		local ret = 0
		local power_type = HDH_POWER[self.unit].power_type;
		local power_max = UnitPowerMax('player', HDH_POWER[self.unit].power_index);
	...
	function HDH_COMBO_POINT_TRACKER:OnEvent(event, unit, powerType)
		...
		if (event == "UNIT_POWER" or event == 'UNIT_MAXPOWER') and (HDH_POWER[self.parent.unit].power_type == powerType) then 
			if not UI_LOCK then
				self.parent:Update()
			end
		end
