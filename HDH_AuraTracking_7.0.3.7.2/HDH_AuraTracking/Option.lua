...
local DDM_BAR_TEXTURE_LIST = {"BantoBar","Minimalist","normTex","Smooth","단색"};
...
local UI_TYPE = { FONT = 1, ICON=2, BAR=3, PROFILE=4, ETC = 5 };
...
local function UpdateFrameDB_DDM(frame, items, value) -- drop down menu
	if frame.db then
		if value ~= nil then
			if frame == FRAME.DDM_UNIT_LIST then
...
function HDH_LoadTrackerListFrame(trackerIdx, startRowIdx, endRowIdx)
	...
	if DB_OPTION[tracker_name] and (HDH_IS_UNIT[unit] or (HDH_T_TRACKER and HDH_T_TRACKER[unit])) and DB_OPTION[tracker_name].tracking_all then
		HDH_AT_NoList:Show()
		...
		while true do
			...
			if aura and aura[i] then
				...
				if unit and string.find(unit,"자원") then 
...
local function HDH_AddRow(rowFrame)
	...
	local tracker_name, unit = HDH_AT_OP_GetTrackerInfo(GetTrackerIndex())
	if name then
	...
	if DB_PROFILE[profile_name].AURA_LIST then
		...
		for i = 1, #p_talent do
			if not list.row[i] then
				...
				new.arrow:SetText("----->");
				new.arrow:SetPoint("LEFT",new,"RIGHT",20,0);
...
function HDH_OnShow_ProfileFrame(self)
	...
	HDH_AT_LoadDropDownButton(FRAME.DDM_PROFILE, nil, dataTable, HDH_AT_OP_OnSelectedItem_DDM)
...
function HDH_OnClick_SaveProfile()
	...
	HDH_OnShow_ProfileFrame(FRAME.DDM_PROFILE)
end
...
function HDH_AT_OP_LoadSetting(curTracker)
	...
	if unit and HDH_IS_UNIT[unit] then FRAME.CB_ABLE_BUFF_CANCEL:Show();
	else FRAME.CB_ABLE_BUFF_CANCEL:Hide(); end
	
	if curTracker and unit and string.find(unit, "쿨다운") then
		UpdateFrameDB_SL(FRAME.SL_CT_MAXTIME, nil, 0, 3000);
...
function HDH_AT_OP_LoadCreateTrackerFrame(trackerIdx)
	if trackerIdx then -- 인덱스가 있으면 수정모드
		...
		UpdateFrameDB_DDM(FRAME.DDM_UNIT_LIST, HDH_TRACKER_LIST);
		...
		if (HDH_T_TRACKER and HDH_T_TRACKER[unit]) then
			FRAME.CB_TRACKER_ALL_AURA:Show();
			UpdateFrameDB_CB(FRAME.CB_TRACKER_ALL_AURA);
		end
		
		if HDH_COMBO_POINT_TRACKER and HDH_COMBO_POINT_TRACKER.POWER[unit] then
		...
		HDH_AT_LoadDropDownButton(FRAME.DDM_UNIT_LIST, nil, HDH_TRACKER_LIST);
		--DDM_LoadUnitList(1)
		--DDM_LoadTabList()
		--DDB_LoadSpecList()
...
function HDH_AT_OP_OnValueChanged(self, value, userInput)
	...
	elseif (self == FRAME.SL_CT_MAXTIME) then
		UpdateFrameDB_SL(FRAME.SL_CT_MAXTIME, value);
		if HDH_AT_OP_IsEachSetting() then
			local t = HDH_TRACKER.Get(name);
			if t then 
				if UI_LOCK then
			...
			if UI_LOCK then
				...
				for k,tracker in pairs(HDH_TRACKER.GetList()) do
					if string.find(tracker.unit, "쿨다운") then
						tracker:Update()
					end
				end
			end
...
function HDH_AT_OP_OnSelectedItem_DDM(self, btn, value, id)
	...
	elseif (self == FRAME.DDM_UNIT_LIST) then
		...
		if (HDH_T_TRACKER and HDH_T_TRACKER[value]) then
			FRAME.CB_TRACKER_ALL_AURA:Show();
			UpdateFrameDB_CB(FRAME.CB_TRACKER_ALL_AURA);
		end
		
		if HDH_COMBO_POINT_TRACKER and HDH_COMBO_POINT_TRACKER.POWER[value] then
			FRAME.CB_TRACKER_MERGE_POWERICON:Show();
		else
			FRAME.CB_TRACKER_MERGE_POWERICON:Hide();
		end
...
function HDH_AT_OP_OnChecked(self, checked)
	...
	elseif self == FRAME.CB_ABLE_BUFF_CANCEL then
		if HDH_IS_UNIT[unit] and not UI_LOCK then
			if HDH_AT_OP_IsEachSetting() then
				if tracker then
					tracker:UpdateSetting()
				end
...
function HDH_AT_UpdatePreviewSplitBar(data)
	...
	local powerMax = UnitPowerMax("player", HDH_POWER_TRACKER.POWER[HDH_TRACKER.Get(name).unit].power_index);
...
function HDH_AT_OnClickBarSplit(self)
	...
	if self == rowFrame.btnAdd then
		...
		if (db[index-1] or 0) < value and value < powerMax then
			...
			HDH_OP_AlertDlgShow(format("%d ~ %d 의 사이값을 입력해주세요.", (db[index-1] or 0), powerMax));
			rowFrame.ed:SetText("");
		end
...
function HDH_LoadSetBarFrame(self, row, spec, idx)  -----------------------------------------
	...
	if HDH_GET_CLASS[HDH_TRACKER.Get(name).unit] == HDH_POWER_TRACKER then
		...
		SplitBarFrame.db = db;
		HDH_AT_UpdatePreviewSplitBar(db);
		RowDetailSetFrameListButton5:Enable();
		SplitBarFrame:Show();
	else
		RowDetailSetFrameListButton5:Disable();
		SplitBarFrame:Hide();
	end
end
...
function HDH_Option_OnLoad(self)
	...
	UI_TAB_BTN_LIST.Body =  { SettingFrameUIBodyFont, SettingFrameUIBodyIcon, SettingFrameUIBodyBar, SettingFrameUIBodyProfile, SettingFrameUIBodyETC };
	UI_TAB_BTN_LIST.CurIdx = 1;
	
	AURA_DETAIL_TAB_BTN_LIST = {RowDetailSetFrameListButton1, RowDetailSetFrameListButton2, RowDetailSetFrameListButton3, RowDetailSetFrameListButton4, RowDetailSetFrameListButton5};
	AURA_DETAIL_TAB_BTN_LIST.Body = {GlowOptionFrame, ValueOptionFrame, ChangeIconFrame, SoundFrame, SettingBarFrame};
