...
function HDH_UserCustomFrameSettings(f)
	...
	f.cooldown1:SetSize(DB_OPTION.icon.size,DB_OPTION.icon.size)
	f.cooldown2:SetSize(DB_OPTION.icon.size,DB_OPTION.icon.size)
	...
	counttext:SetPoint('TOPRIGHT', f, 'TOPRIGHT', 0, -1)
	--counttext:SetPoint('BOTTOM', timetext, 'TOP')
	...
	timetext:SetHeight(DB_OPTION.icon.size)
	...
	timetext:SetTextColor(unpack(DB_OPTION.font.textcolor))
	timetext:SetNonSpaceWrap(false)
	timetext:SetPoint('BOTTOMLEFT', f, 'BOTTOMLEFT', 0, -1)
...
function HDH_UpdateAura(unit)
	...
	for i = 1, 40 do 
		...
		f = AuraPointer[unit][tonumber(id)]
