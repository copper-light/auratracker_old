...
function HDH_DB_SaveSpell(key, spec, no, id, name, fix, glow,unit)
...
function HDH_AddRow(rowFrame)
	...
	if name then
		if not HDH_DB_SaveSpell(value, CurSpec, no, id, name, fix, glow, unit) then
			print("|cffff0000Error - |cffffff00'"..name.."("..id..")' is already registered.")
			return nil
		end
		HDH_SetRowData(rowFrame, value, no, id, name, fix, glow)
	else
...
function HDH_SetRowData(rowFrame, key, no, id, name, fix, glow)
...
function HDH_LoadListFrame(unit, startRowIdx, endRowIdx)
	...
	while true do
		...
		if aura and aura[i] then
			HDH_SetRowData(rowFrame, aura[i].Key, aura[i].No, aura[i].ID, aura[i].Name, aura[i].Fix, aura[i].Glow)
...
function HDH_AddUnitTabButton(unit)
	...
	_G[btn:GetName().."Text"]:SetText(unit:gsub("^%l", string.upper))
	if #parent.tablist == 0 then btn:SetPoint("TOPLEFT", btn:GetParent(), "BOTTOMLEFT", 0, 4)
									 else btn:SetPoint("LEFT", parent.tablist[#parent.tablist], "RIGHT", -15, 0) end
	btn:SetScript("OnClick", HDH_OnTabClick)
	btn.unit = unit
...
function HDH_LoadTabUnit()
	for i = 1, #UNIT_LIST do
		if UNIT_LIST[i] == CurUnit then
			HDH_ChangeUnitTab(UnitOptionFrame.tablist[i])
			break
		end
	end
end

function HDH_OnTabClick(self) -- script 펑션은 후킹이 안됨
	HDH_ChangeUnitTab(self) -- hooking 가능 하도록
end

function HDH_ChangeUnitTab(self)
	CurUnit = self.unit
	if CurUnit == 'player' then _G["UnitOptionFrameCheckButtonPlayerDebuff"]:Show()
								_G["UnitOptionFrameCheckButtonPet"]:Show()
						   else _G["UnitOptionFrameCheckButtonPlayerDebuff"]:Hide() 
								_G["UnitOptionFrameCheckButtonPet"]:Hide() end
						   
						   
	if not UnitOptionFrame.tablist then return end
	for i=1, #UnitOptionFrame.tablist do
		if UnitOptionFrame.tablist[i].unit == CurUnit then
			SetActiveUnitTab(self, true)
			HDH_LoadListFrame(CurUnit)
			HDH_LoadUnitUI(CurUnit)
			UnitOptionFrameTextTitle:SetText(CurUnit:gsub("^%l", string.upper))
			if self.desc then
				UnitOptionFrameTextTitle2:SetText(self.desc)
				...
				UnitOptionFrameTextTitle2:SetText(nil)
			end
		else
...
function HDH_RefrashSettingUI()
	...
	_G["SettingFrameButtonColorCooldownBgColor"]:SetTexture(unpack(DB_OPTION.icon.cooldown_bg_color))
...
function HDH_OnSelectedColorAlpha()
	...
	DB_OPTION.icon.cooldown_bg_color = {r, g, b, OpacitySliderFrame:GetValue()}
	_G["SettingFrameButtonColorCooldownBgColor"]:SetTexture(unpack(DB_OPTION.icon.cooldown_bg_color))
	HDH_UpdateSetting()
end

function HDH_OnSelectColorCancel()
	DB_OPTION.icon.cooldown_bg_color[1] = ColorPickerFrame.previousValues[1]
	DB_OPTION.icon.cooldown_bg_color[2] = ColorPickerFrame.previousValues[2]
	DB_OPTION.icon.cooldown_bg_color[3] = ColorPickerFrame.previousValues[3]
	DB_OPTION.icon.cooldown_bg_color[4] = ColorPickerFrame.previousValues[4]
	_G["SettingFrameButtonColorCooldownBgColor"]:SetTexture(unpack(DB_OPTION.icon.cooldown_bg_color))
	HDH_UpdateSetting()
end
