...
function HDH_InitVaribles()
	-- 세팅 정보 저장하는 세이브인스턴스
	if not AuraTracking or not AuraTracking.HDH_AT_VERSION or (tonumber(AuraTracking.HDH_AT_VERSION) < HDH_AT_VERSION) then
		...
		AuraTracking.HDH_AT_VERSION = HDH_AT_VERSION
	end
...
local function AllFrameVisible(show)
	...
	--[[if show == true then
		for t,k in pairs(Icons.Buff) do
			k:Show()
		end
		for t,k in pairs(Icons.Debuff) do
			k:Show()
		end
	else
		for t,k in pairs(Icons.Buff) do
...
local function GetUnitBuffByID(unit, spell_id)
	if spell_id then
		local name, _, _, count, _, duration, endTime, caster, _, _, id = UnitBuff(unit, GetSpellInfo(spell_id))
		if id and id == spell_id then
			return name, count, duration, endTime, caster
		end
	end
	return nil
end

local function GetUnitDebuffByID(unit, spell_id)
	if spell_id then
		local name, _, _, count, _, duration, endTime, caster, _, _, id = UnitDebuff(unit, GetSpellInfo(spell_id))
		if id and id == spell_id then
			return name, count, duration, endTime, caster
		end
	end
...
function HDH_UpdateAuraIcons(unit)
	-- 순서 정하기 구문 필요 		
	for k,f in pairs(AuraPointer[unit]) do
		if f.spell.isUpdate then
			-- 위치 지정
			if f.spell.count < 2 then
...
function HDH_UpdateAura(unit)
	...
	local spell
	local f
	if not AuraPointer[unit] then return end
	for i = 1, 40 do 
		...
		if f then
			...
			spell.count = count
			spell.id = id
			spell.reamaining = endTime - GetTime()
...
function HDH_InitAuraIcon()
	...
	HDH_UpdateAura("player")
	HDH_UpdateAura("target")
	--AllFrameVisible(UnitAffectingCombat("player"))
end
