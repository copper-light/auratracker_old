...
local show
local update
-- 쿨다운 시간 제한 옵션 때문에 매초 마다 쿨다운 체크해서 화면에 출력할지 말지를 정해야함
-- 하지만 매초 시간을 체크하는건 비효율적이고 cpu 사용량도 높아서 3초마다 체크함.
-- 그로인해서 정교함이 떨어지지만 시간 제한 기능에서의 정교함은 중요한 가치가 아니라고 봄
function ATC_OnUpdate_ChackTime() -- timer 로 만들어야 하나?
	cur = GetTime()
	if (cur - tmpdelay) > 3 then -- 효율상 3초마다 체크
		show = false
		update = false
		for i = 1, #ATC.ICONS do
			if ATC.ICONS[i] and ATC.ICONS[i].spell then
				ATC.ICONS[i].spell.remaining = ATC.ICONS[i].spell.endTime - cur -- 남은 시간 체크해서
				--print(ATC.ICONS[i].spell.name, ATC.ICONS[i].spell.remaining)
				if ATC.OPTION.max_time ~= 0  -- 제한있고 아니고
						and not ATC.ICONS[i].spell.fix
						and ATC.OPTION.max_time > ATC.ICONS[i].spell.remaining -- 제한 시간 보다 적은 시간의 스킬이 있으면
						and ATC.ICONS[i].spell.remaining > 0 then 
					if ATC:Update_Icon(ATC.ICONS[i]) then update = true end -- 아이콘의 위치 변경할일이 생겼나?
				end
		...
		--[[print(show, DB_OPTION.cooldown.always_show , UnitAffectingCombat("player"))
