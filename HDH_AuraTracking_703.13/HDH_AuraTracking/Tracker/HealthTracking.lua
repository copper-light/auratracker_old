...
local HDH_POWER = {}
HDH_HEALTH_TRACKER.DATA = {type_name="HEALTH", bar_color={0, 1, 0}, texture = "Interface\\Icons\\INV_Misc_Rune_03"};
...
------------------------------------
	...
	function HDH_HEALTH_TRACKER:Update() -- HDH_TRACKER override
		...
		if f and f.spell then
			...
			self:UpdateIcons()
			if f.spell.v1 < f.spell.max then show = true end
		end
	...
	function HDH_HEALTH_TRACKER:InitIcons() -- HDH_TRACKER override
		...
		if self:IsHaveData(self:GetSpec()) then
			...
			-- spell.type = HDH_HEALTH_TRACKER.DATA = {type_name
			...
			self.frame:RegisterUnitEvent("UNIT_HEAL_PREDICTION", "player")
			-- GetPowerRegen
			self:Update()
		else
