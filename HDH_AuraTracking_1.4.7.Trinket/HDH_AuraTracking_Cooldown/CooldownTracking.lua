...
function InitVariables()
	...
	if nil == DB_OPTION.cooldown.cooldown_bg_color then 
		DB_OPTION.cooldown.cooldown_bg_color = {0,0,0,0.75}
	end
end
...
function ATC:InitIcons()
	...
	for i = 1 , #self.DATA do
		...
		iconFrame.spell.glow = db_spell.Glow
		...
		if iconFrame.spell.isItem and self.DATA[i].trinket then
			iconFrame.spell.trinket = {}
			iconFrame.spell.trinket.duration = self.DATA[i].trinket.duration
			iconFrame.spell.trinket.key = self.DATA[i].trinket.key
			iconFrame.spell.trinket.cooldown = self.DATA[i].trinket.cooldown
			iconFrame:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED");
			iconFrame:UnregisterEvent("ACTIONBAR_UPDATE_STATE");
			iconFrame:UnregisterEvent("ACTIONBAR_UPDATE_USABLE");
			iconFrame:UnregisterEvent("ACTIONBAR_UPDATE_COOLDOWN");
			iconFrame:UnregisterEvent("SPELL_ACTIVATION_OVERLAY_GLOW_SHOW");
			iconFrame:UnregisterEvent("SPELL_ACTIVATION_OVERLAY_GLOW_HIDE");
		else
			iconFrame:UnregisterEvent("COMBAT_LOG_EVENT_UNFILTERED");
...
function ATC_OnUpdateCooldown(self)
	...
	if spell.remaining > 0.1 and spell.duration > 0 then
		...
		ATC:SetChangeAble(self:GetParent(), true)
		self:GetParent():Show()
		--ATC:Update_Layout()
	end
...
function ATC:UserCustomFrameSettings(f)
	...
	f.cooldown1:SetStatusBarTexture(unpack(self.OPTION.cooldown_bg_color))
	f.cooldown2:SetSwipeColor(unpack(self.OPTION.cooldown_bg_color))
...
function ATC:SetChangeAble(f, value)
	if value then
		...
		local desaturated = self.OPTION.use_able
		if desaturated then f.icon:SetDesaturated(1)
...
function ATC:UpdateIcons()
	...
	for i = 1 , #self.ICONS do
		if self.ICONS[i].spell.trinket then
			isUpdateLayout = self:Update_Icon_Trinket(self.ICONS[i])
		else
...
function ATC:Update_Icon(f)
	...
	if ((spell.duration > 2) or not spell.isAble) and not spell.isCharging then -- 글로버 쿨다운 2초  무시
		...
		if not ATC_HasTimer() then ATC_StartTimer() end
...
function ATC_OnEventIcon(this,event,...)
	...
	elseif event == "COMBAT_LOG_EVENT_UNFILTERED" then
		ATC_COMBAT_LOG_EVENT_UNFILTERED(this,...)
	end
end
...
function ATC_OnEvent(this,event,...)
	...
	elseif event == "PLAYER_ENTERING_WORLD" then
		...
		C_Timer.After(4, ATC_PLAYER_ENTERING_WORLD) -- GET_ITEM_INFO_RECEIVED 이게 이상하게 일은 안해서.. 2초 딜레이후에 아이템 데이터들 가져옴
	end
end	
...
function ATC:Update_Icon_Trinket(f)
	if not self.OPTION then return false end
	local option = self.OPTION
	local spell = f.spell
	...
	if spell.isActive then
		...
		f.cooldown1:SetStatusBarTexture(0,0,0,0)
...
local function CombatLog_UpdateTrinket(self, id, name)
	if not self or not self.spell or not self.spell.trinket then return end
	
	local trinket = self.spell.trinket
	local spell = self.spell
	if tonumber(trinket.key) == id or trinket.key == name then
		spell.duration = trinket.duration
		spell.startTime = GetTime()
		spell.endTime = spell.startTime + spell.duration
		spell.isActive = true
		if ATC:Update_Icon_Trinket(self) then
			ATC:Update_Layout()
		end
	end
end

local function CombatLog_RemoveTrinket(self, id, name)
	if not self or not self.spell or not self.spell.trinket then return end
	
	local trinket = self.spell.trinket
	local spell = self.spell
	if tonumber(trinket.key) == id or trinket.key == name then
		spell.duration = (trinket.cooldown*60) - (trinket.duration- (spell.remaining or 0))
		spell.startTime = GetTime()
		spell.endTime = spell.startTime + spell.duration
...
local function CombatLog_UpdateTrinketCount(self, id, name, count)
	if not self or not self.spell or not self.spell.trinket then return end
...
local CombatLogEventList = {
	["SPELL_AURA_APPLIED"]		= CombatLog_UpdateTrinket,
	["SPELL_AURA_REFRESH"] 		= CombatLog_UpdateTrinket,
	["SPELL_AURA_BROKEN"] 		= CombatLog_RemoveTrinket,
	["SPELL_AURA_BROKEN_SPELL"] = CombatLog_RemoveTrinket,
	["SPELL_AURA_REMOVED"] 		= CombatLog_RemoveTrinket,
	["SPELL_AURA_APPLIED_DOSE"] = CombatLog_UpdateTrinketCount,
	["SPELL_AURA_REMOVED_DOSE"] = CombatLog_UpdateTrinketCount,
}

function ATC_COMBAT_LOG_EVENT_UNFILTERED(self, ...)
	...
	if UnitGUID('player') == destGUID  then
		...
		if func then
			func(self, spellid, spellname, stackCount)
		end
	end
end