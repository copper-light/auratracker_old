...
--------------------------------------------
	...
	function HDH_TRACKER.InitVaribles()
		...
		for i = 1, #DB_FRAME_LIST do
			...
			if not tracker then
				if HDH_TRACKER.new(DB_FRAME_LIST[i].name, DB_FRAME_LIST[i].unit) == nil then
					local newUnit = HDH_PARSE_OLD_TRACKER[DB_FRAME_LIST[i].unit];
					if newUnit then
						DB_FRAME_LIST[i].unit = newUnit;
						HDH_TRACKER.new(DB_FRAME_LIST[i].name, DB_FRAME_LIST[i].unit);
					else
						for j = i, #DB_FRAME_LIST-1 do
							DB_FRAME_LIST[j] = DB_FRAME_LIST[j+1];
							DB_FRAME_LIST[j+1] = nil;
						end
						DB_FRAME_LIST[#DB_FRAME_LIST] = nil;
					end
				end
			else
