...
local F_OP_DDM_CD_TYPE;
	...
	-- 스킬 쿨다운 관련 설정 --
local F_OPCT_CP_COLOR;
local F_OPCT_CB_DESAT;
local F_OPCT_SL_MAXTIME;
local F_OPCT_DDM_RELATIVE_UNIT;
...
local function InitFrame()
	...
	-- 스킬 쿨다운 관련 설정 --
	F_OPCT_CP_COLOR = CooldownSettingFrameButtonColor;
	F_OPCT_CP_COLOR.key = "cooldown_color";
	F_OPCT_CB_DESAT = CooldownSettingFrameCheckButtonDesaturation;
	F_OPCT_CB_DESAT.key = "desaturation";
	F_OPCT_SL_MAXTIME = CooldownSettingFrameSliderMaxTime;
	F_OPCT_SL_MAXTIME.key = "max_time";
end
...
local function Match_FontIcon_DBForFrame(curTracker)
	...
	F_OPCT_CP_COLOR.db = icon;
	F_OPCT_CB_DESAT.db = icon;
	F_OPCT_SL_MAXTIME.db = icon;
end
...
local function HDH_DB_SaveSpell(key, spec, no, id, name, always, glow, showValue, texture, isItem, tabIdx)
...
local function HDH_SetRowData(rowFrame, key, no, id, name, always, glow, showValue, texture, isItem)
...
local function HDH_AddRow(rowFrame)
	...
	if name then
		if not HDH_DB_SaveSpell(key, CurSpec, no, id, name, always, glow, showValue, icon, isItem, GetTrackerIndex()) then
...
function HDH_AT_OP_LoadSetting(curTracker)
	...
	if not curTracker or string.find(unit, "cooldown") then
		UpdateFrameDB_SL(F_OPCT_SL_MAXTIME, nil, 0, 3000);
		UpdateFrameDB_CB(F_OPCT_CB_DESAT);
		UpdateFrameDB_CP(F_OPCT_CP_COLOR);
		F_OPCT_SL_MAXTIME:Show();
		F_OPCT_CB_DESAT:Show();
		F_OPCT_CP_COLOR:Show();
	else
		F_OPCT_SL_MAXTIME:Hide();
		F_OPCT_CB_DESAT:Hide();
		F_OPCT_CP_COLOR:Hide();
	end
end
...
function HDH_OnRevers(self, checked)
...
function HDH_OnShowSpellTooltip(self, checked)
	local name = HDH_AT_OP_GetTrackerInfo(GetTrackerIndex())
	if not name then return end
	UpdateFrameDB_CB(self, checked);
	if not UI_LOCK then
...
function HDH_OnCheckShowCooldown(self, checked)
	UpdateFrameDB_CB(self, checked);
	if HDH_AT_OP_IsEachSetting() then
		local t = HDH_TRACKER.Get(HDH_AT_OP_GetTrackerInfo(GetTrackerIndex()))
		if t then
			t:UpdateSetting()
		end
	else
...
function HDH_OnFix(self, checked)
	local name = HDH_AT_OP_GetTrackerInfo(GetTrackerIndex())
	if not name then return end
	UpdateFrameDB_CB(self, checked);
	local t = HDH_TRACKER.Get(name)
...
function HDH_AT_OP_OnValueChanged(self, value, userInput)
	...
	elseif (self == F_OPCT_SL_MAXTIME) then
		UpdateFrameDB_SL(F_OPCT_SL_MAXTIME, value);
		if HDH_AT_OP_IsEachSetting() then
			local t = HDH_TRACKER.Get(name);
			if t then 
				if UI_LOCK then
...
function HDH_AT_OP_OnCheckedDesaturation(self, checked)
	UpdateFrameDB_CB(self,checked);
	if HDH_AT_OP_IsEachSetting() then
		local t= HDH_TRACKER.Get( HDH_AT_OP_GetTrackerInfo(GetTrackerIndex()))
		if t then 
...
function HDH_OnCheckTooltipShow(self, checked)
	UpdateFrameDB_CB(self, checked);
end

function HDH_OnAlwaysShow(self, checked)
	UpdateFrameDB_CB(self, checked);
	for k,tracker in pairs(HDH_TRACKER.GetList()) do
		tracker:Update()
	end
...
function HDH_OnCheckedDefaultColor(self, checked)
	local name = HDH_AT_OP_GetTrackerInfo(GetTrackerIndex())
	local tracker = HDH_TRACKER.Get(name)
	UpdateFrameDB_CB(self, checked);
	if tracker then
...
function HDH_OnChecked_EachSet(self, checked)
	local name = HDH_AT_OP_GetTrackerInfo(GetTrackerIndex());
	local tracker = HDH_TRACKER.Get(name);
	UpdateFrameDB_CB(F_OP_CB_EACH, checked);
	if not tracker then return; end
	if checked then
...
function HDH_Option_OnShow(self)
	...
	HDH_AT_OP_ChangeTracker(GetTrackerIndex());
end
