...
do 
	...
	function HDH_C_TRACKER:CreateDummySpell(count)
		...
		for i=1, count do
			...
			spell.glow = false
			spell.count = 3
			spell.duration = ((i > 1) and (30 * i)) or 0
			spell.remaining = 0
	...
	function HDH_C_TRACKER:Update() -- HDH_TRACKER override
		...
		if not UI_LOCK and self.option.base.relative_unit and not UnitExists(self.option.base.relative_unit) then
	...
	function HDH_C_TRACKER:InitIcons() -- HDH_TRACKER override
		...
		if #(self.frame.icon) > 0 and self.option.base.relative_unit then
			...
			elseif string.find(self.option.base.relative_unit, 'arena') then
