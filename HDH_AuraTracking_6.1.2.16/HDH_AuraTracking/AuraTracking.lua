...
--------------------------------------------
	...
	local function GetAuras(self, filter, isbuff)
		...
		for i = 1, 40 do 
			name, _, _, count, _, duration, endTime, caster, _, _, id, _, _, _, v2, v3 = UnitAura(self.unit, i, filter)
	...
	local function GetAurasForResolveAndStagger(self, filter, isbuff) 
		local curTime = GetTime()
		for i = 1, 40 do 
			name, _, _, count, _, duration, endTime, caster, _, _, id, canApplyAura, isBossDebuff, v1,v2,v3 = UnitAura(self.unit, i, filter)
			if not id then break end
			f = self.frame.pointer[tostring(id)] or self.frame.pointer[name]
			if f and f.spell then
				spell = f.spell
				if StaggerID[id] then
					spell.count = math.floor(v3/UnitHealthMax("player")*100)
					spell.isUpdate = true
				elseif ResolveID[spell.name] and ResolveID[id] then
					spell.count = v2
					spell.isUpdate = (spell.count > 0)
				else
					spell.count = spell.count + count
					spell.isUpdate = true
				end
	...
	local function GetAurasForIgnite(self, filter, isbuff)
		local curTime = GetTime()
		for i = 1, 40 do 
			name, _, _, count, _, duration, endTime, caster, _, _, id, canApplyAura, isBossDebuff, v1,v2,v3 = UnitAura(self.unit, i, filter)
			if not id then break end
	...
	local function GetAurasAll(self, filter, isbuff) 
		...
		local ret = 1
		for i = ret, 40 do 
			name, _, icon, count, _, duration, endTime, caster, _, _, id, canApplyAura, isBossDebuff, v1,v2,v3 = UnitAura(self.unit, i, filter)
	...
	function HDH_TRACKER:Update()
		...
		if self.option.base.check_debuff then 
			GetAuras(self, "HARMFUL"..(mine or ""), false)
		end
		
		if self.option.base.check_buff then 
			GetAuras(self, "HELPFUL", true)
		end
	...
	function HDH_TRACKER:InitIcons()
		...
		if self.option.base.tracking_all then
			...
			if isTanker then
				self.GetAurasFunc = GetAurasForResolveAndStagger
			elseif isIgnite then
				self.GetAurasFunc = GetAurasForIgnite
			else
	...
	-- 애니메이션 관련
	-------------------------------------------
	function HDH_TRACKER:ActionButton_ShowOverlayGlow(self)
