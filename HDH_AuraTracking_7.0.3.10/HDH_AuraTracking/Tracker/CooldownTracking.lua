...
do 
	...
	function HDH_C_TRACKER:OnUpdateBarValue(elapsed)
		...
		elseif HDH_TRACKER.IsEqualClass(tracker.type, "HDH_C_TRACKER") then
			-- self:Hide();
			if( tracker:Update_Icon(self:GetParent())) or (not tracker.option.icon.always_show and not UnitAffectingCombat("player")) then
				tracker:Update_Layout()
			end
		end
	end
	...
	function HDH_C_TRACKER:Update_Icon(f) -- f == f
		...
		if (spell.duration > 0) or (spell.charges.duration > 0) then -- 글로버 쿨다운 2초 포함
			if (spell.duration < HDH_C_TRACKER.GlobalCooldown) and spell.charges.duration == 0 then
				...
				if (option.icon.max_time == -1) or (option.icon.max_time > spell.remaining) then
			...
			if spell.isAble and spell.always then 
				if not f:IsShown() then f:Show() self:SetGlow(f, spell.glow); isUpdate= true end
			else 
		...
		-- if self.frame.icon then
			-- for i= 1, #self.frame.icon do
				-- if self.frame.icon[i] then
					...
					-- print(slot,slot_id)
					-- if ADJUST_ID[slot_id] and ADJUST_ID[slot_id] == ADJUST_ID[self.frame.icon[i].spell.id] then
