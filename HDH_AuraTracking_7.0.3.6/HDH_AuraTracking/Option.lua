...
local F_OP_CP_CT_NOTENOUGHMANA;
local F_OP_CP_CT_OUTRANGE;
local F_OP_SL_CT_MAXTIME;
local F_OP_DDM_CT_RELATIVE_UNIT;
local F_OP_CB_CT_DESAT;
local F_OP_CB_CT_DESAT_NOTENOUGHMANA;
local F_OP_CB_CT_DESAT_OUTRANGE;
...
local function InitFrame()
	...
	F_OP_CB_CT_DESAT_NOTENOUGHMANA = CooldownSettingFrameCheckButtonDesaturationNotEnoughMana;
	F_OP_CB_CT_DESAT_NOTENOUGHMANA.key = "desaturation_not_mana";
	F_OP_CB_CT_DESAT_OUTRANGE = CooldownSettingFrameCheckButtonDesaturationOutRange;
	F_OP_CB_CT_DESAT_OUTRANGE.key = "desaturation_out_range";
	F_OP_SL_CT_MAXTIME = CooldownSettingFrameSliderMaxTime;
	F_OP_SL_CT_MAXTIME.key = "max_time";
	F_OP_CP_CT_NOTENOUGHMANA = CooldownSettingFrameButtonColorNotEnoughMana;
	F_OP_CP_CT_NOTENOUGHMANA.key = "not_enough_mana_color";
	F_OP_CP_CT_OUTRANGE = CooldownSettingFrameButtonColorOutRange;
	F_OP_CP_CT_OUTRANGE.key = "out_range_color";
end
...
local function Match_FontIcon_DBForFrame(curTracker)
	...
	F_OP_CB_CT_DESAT_OUTRANGE.db = icon;
	F_OP_CB_CT_DESAT_NOTENOUGHMANA.db = icon;
	F_OP_SL_CT_MAXTIME.db = icon;
	F_OP_CP_CT_NOTENOUGHMANA.db = icon;
	F_OP_CP_CT_OUTRANGE.db = icon;
end
...
function HDH_AT_OP_LoadSetting(curTracker)
	...
	if curTracker and string.find(unit, "cooldown") then
		UpdateFrameDB_SL(F_OP_SL_CT_MAXTIME, nil, 0, 3000);
		UpdateFrameDB_CB(F_OP_CB_CT_DESAT);
		UpdateFrameDB_CB(F_OP_CB_CT_DESAT_NOTENOUGHMANA);
		UpdateFrameDB_CB(F_OP_CB_CT_DESAT_OUTRANGE);
		UpdateFrameDB_CP(F_OP_CP_CT_COLOR);
		UpdateFrameDB_CP(F_OP_CP_CT_NOTENOUGHMANA);
		UpdateFrameDB_CP(F_OP_CP_CT_OUTRANGE);
		-- F_OP_SL_CT_MAXTIME:Show();
		-- F_OP_CB_CT_DESAT:Show();
		-- F_OP_CP_CT_COLOR:Show();
		CooldownSettingFrame:Show();
		
		F_OP_CB_COLOR_DEBUFF_DEFAULT:Hide();
		F_OP_BTN_COLOR_BUFF:Hide();
		F_OP_BTN_COLOR_DEBUFF:Hide();
		--F_OP_BTN_COLOR_CD_BG:Hide();
	else
		CooldownSettingFrame:Hide();
		-- F_OP_SL_CT_MAXTIME:Hide();
		-- F_OP_CB_CT_DESAT:Hide();
		-- F_OP_CP_CT_COLOR:Hide();
		F_OP_CB_COLOR_DEBUFF_DEFAULT:Show();
		F_OP_BTN_COLOR_BUFF:Show();
		F_OP_BTN_COLOR_DEBUFF:Show();
		--F_OP_BTN_COLOR_CD_BG:Show();
	end
end
...
function HDH_AT_OP_OnChecked(self, checked)
	...
	elseif self ==  F_OP_CB_CT_DESAT or self == F_OP_CB_CT_DESAT_NOTENOUGHMANA or self == F_OP_CB_CT_DESAT_OUTRANGE then
		if HDH_AT_OP_IsEachSetting() then
			if tracker then 
				if UI_LOCK then
					tracker:SetMove(UI_LOCK)
