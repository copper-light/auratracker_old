...
local function OnUpdateCooldown(self)
	...
	if spell.remaining > 0.0 and spell.duration > 0 then
		...
		-- if spell.remaining <= 0.1 then
			-- self:GetParent():GetParent().parent:StartAni(self:GetParent(),1)
		-- else
			-- self:GetParent():GetParent().parent:StopAni(self:GetParent())
		-- end
		
		if spell.remaining > 60 then
			self:GetParent().timetext:SetText(('%d:%02d'):format((spell.remaining)/60, (spell.remaining)%60))
		else
			self:GetParent().timetext:SetText(('%d'):format(spell.remaining))
		end
...
local function frameBaseSettings(f)
	...
	-- end);
	f.topframe = CreateFrame("Frame", f:GetName().."Top", f);
	f.topframe:SetPoint('TOPLEFT', f, 'TOPLEFT', 0, 0)
	f.topframe:SetPoint('BOTTOMRIGHT', f, 'BOTTOMRIGHT', 0, 0)
	f.topframe:Show();
	
	f.icon = f.topframe:CreateTexture(nil, 'BACKGROUND')
	f.icon:SetTexCoord(0.08, 0.92, 0.08, 0.92)
	f.icon:SetPoint('TOPLEFT', f.topframe, 'TOPLEFT', 0, 0)
	f.icon:SetPoint('BOTTOMRIGHT', f.topframe, 'BOTTOMRIGHT', 0, 0)
	
	f.cooldown1 = CreateFrame("StatusBar", nil, f)
	f.cooldown1:SetScript('OnUpdate', OnUpdateCooldown)
	f.cooldown1:SetPoint('TOPLEFT', f.topframe, 'TOPLEFT', 0, 0)
	f.cooldown1:SetPoint('BOTTOMRIGHT', f.topframe, 'BOTTOMRIGHT', 0, 0)
	f.cd = f.cooldown1
	
	f.cooldown2 = CreateFrame("Cooldown", nil, f) -- 원형
	f.cooldown2:SetPoint('TOPLEFT', f.topframe, 'TOPLEFT', 0, 0)
	f.cooldown2:SetPoint('BOTTOMRIGHT', f.topframe, 'BOTTOMRIGHT', 0, 0)
...
--------------------------------------------
	...
	function HDH_TRACKER.InitVaribles()
		...
		if not DB_OPTION or not DB_OPTION.HDH_AT_DB_VERSION or (tonumber(DB_OPTION.HDH_AT_DB_VERSION) ~= HDH_AT_DB_VERSION) then
			DB_OPTION = { tooltip_id_show = true,
		...
		if DB_OPTION.icon.cooldown_color == nil then
			DB_OPTION.icon.cooldown_color = {0,0,0};
			DB_OPTION.icon.desaturation = false;
			DB_OPTION.icon.max_time = 0;
		end
...
--------------------------------------------
	...
	function HDH_TRACKER:InitVariblesOption()
		...
		if self.option.icon.always_show == nil then
			self.option.icon.always_show = false;
		end
	...
	function HDH_TRACKER:InitVariblesAura()
		...
		for i =1, 4 do
			...
			if id then
				...
				if DB_AURA.Talent[i][self.name].tracker_enable == nil then
					DB_AURA.Talent[i][self.name].tracker_enable = true;
				end
			end
		end
	end
	...
	local function ChangeFontLocation(parent, fontf, location, op_font)
		...
		elseif location == FONT_LOCATION_BL then
			...
			fontf:SetPoint('BOTTOMRIGHT', parent, 'BOTTOMRIGHT', 50, 1)
	...
	function HDH_TRACKER:UpdateIconSettings(f)
		...
		f.topframe:SetSize(op_icon.size,op_icon.size);
		...
		f.border:SetPoint('CENTER', f.topframe, 'CENTER', 0, 0)
	...
	function HDH_TRACKER:SetMove(move)
		...
		if not self.enable then self.frame:Hide(); return end
		if move then
			...
			local cnt = (self.option.base.tracking_all and HDH_TRACKER.MAX_ICONS_COUNT or 1)
		...
		for k,f in ipairs(icons) do
			...
			if f.spell.isUpdate then
				...
				if isBuff then f.border:SetVertexColor(unpack(self.option.icon.buff_color)) 
						  else if self.option.icon.defaultColor then f.border:SetVertexColor(DebuffTypeColor[f.spell.dispelType or ""].r,
			...
			-- if f.spell.aniEnable then
				-- if f.spell.aniOverSec then
				
				-- else
					-- if f.spell.endTime then
						-- AT_StartTimer(f, f.spell.endTime-f.spell.aniTime);
					-- else
						-- self:StartAni(f,1); -- 이게 문제
					-- end
				-- end	
			-- end
			f.spell.overlay = 0
	...
	-- local function GetAurasBossDebuff(self) 
		-- local curTime = GetTime()
		-- local ret = 1;
		-- for i = 1, 40 do 
			-- name, _, icon, count, dispelType, duration, endTime, caster, _, _, id, canApplyAura, isBossDebuff = UnitAura(self.unit, i, self.filter)
	...
	function HDH_TRACKER:Update()
		...
		if not self.enable or not UnitExists(self.unit) or not self.frame.pointer or not self.option then 
	...
	function HDH_TRACKER:InitIcons()
		...
		self.frame.pointer = {}
		if auraList.tracker_enable == nil then auraList.tracker_enable = true; end
		self.enable = auraList.tracker_enable;
		if not self.enable then self.frame:UnregisterAllEvents() self.frame:Hide() return end
		...
		-- elseif self.option.base.boss_debuff then
			-- self.GetAurasFunc = GetAurasBossDebuff
			-- if #(self.frame.icon) > 0 then
				-- self:ReleaseIcons()
			-- end
		else
			for i = 1, #auraList do--[[
				...
				-- iconFrame:SetScript("OnUpdate",function(self)
					-- if (self.spell.remaining or 0) <5 then
						-- self.topframe:ClearAllPoints();
						-- self.topframe:SetSize(self:GetWidth(),self:GetHeight());
						-- self.topframe.y = (GetTime()*70)%50;
						-- if self.topframe.y > 25 then
							-- self.topframe:SetPoint("CENTER",self,"CENTER",0, 50-self.topframe.y);
						-- else
							-- self.topframe:SetPoint("CENTER",self,"CENTER",0, self.topframe.y);
						-- end
						
					-- else
						-- self.topframe:SetAllPoints();
					-- end
				-- end);
	...
	function HDH_TRACKER:ActionButton_ShowOverlayGlow(f)
		...
		-- else
			...
			-- f.overlay:SetSize(frameWidth * 1.60, frameHeight * 1.60);
	...
	local function CreateAni(f, type) -- row 이동 애니
		if f.ani then return end
		local ag = f:CreateAnimationGroup();
		local move = (f:GetHeight()/3);
		-- if not ag.a1 then
			ag.a1 = ag:CreateAnimation("Translation");
			ag.a1:SetOrder(1)
			ag.a1:SetDuration(0.2)
			ag.a1:SetSmoothing("OUT")
			ag.a2 = ag:CreateAnimation("Translation");
			ag.a2:SetOrder(2)
			ag.a2:SetDuration(0.2)
			ag.a2:SetSmoothing("IN")
		-- end
		-- if type == 1 then -- up
			ag.a1:SetOffset(0, move);
			ag.a2:SetOffset(0, -move);
		-- elseif type ==2 then --down
			-- ag.a1:SetOffset(0, -move);
		-- elseif type ==3 then -- left
			-- ag.a1:SetOffset(-move, 0);
		-- else -- right
			-- ag.a1:SetOffset(move, 0);
		-- end
		...
		-- ag.ap:SetOrder(1)
		-- ag.ap:SetDuration(0.4)
		-- ag.ap:SetSmoothing("OUT")
		-- ag.ap:SetFromAlpha(1)
		-- ag.ap:SetToAlpha(0.2) 
		
		-- ag.sc = ag:CreateAnimation("Scale");
		-- ag.sc:SetOrigin("CENTER", 0, 0)
		-- ag.sc:SetOrder(1)
		-- ag.sc:SetDuration(1)
		-- ag.sc:SetScale(1.5, 1.5)
		
		-- ag.a2 = ag:CreateAnimation("Translation")
		-- ag.a2:SetOrder(2)
		-- ag.a2:SetDuration(0.5)
		-- ag.a2:SetSmoothing("OUT")
		-- ag.a2:SetOffset(0, -move)
		
		-- ag.a3 = ag:CreateAnimation("Translation")
		-- ag.a3:SetOrder(3)
		-- ag.a3:SetDuration(0.5)
		-- ag.a3:SetSmoothing("OUT")
		-- ag.a3:SetOffset(0, -move)
		
		-- ag.a4 = ag:CreateAnimation("Translation")
		-- ag.a4:SetOrder(4)
		-- ag.a4:SetDuration(0.5)
		-- ag.a4:SetSmoothing("OUT")
		-- ag.a4:SetOffset(-move, 0)
	...
	function HDH_TRACKER:StartAni(f, ani_type) -- row 이동 실행
		if not f.ani then CreateAni(f, ani_type) 
		else
			if f.ani_type ~= ani_type then
				self:StopAni(f);
				CreateAni(f, ani_type);
			end
		end
		f.ani:Play();
		-- f.cooldown2:SetDrawSwipe(false);
	end
	
	function HDH_TRACKER:StopAni(f) -- row 이동 실행
...
HDH_AT_ADDON_Frame = CreateFrame("Frame", "HDH_AT_TOPFrame", UIParent) -- 애드온 최상위 프레임
