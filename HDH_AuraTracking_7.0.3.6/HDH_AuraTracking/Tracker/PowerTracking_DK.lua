...
------------------------------------
	...
	function HDH_POWER_DK_TRACKER:Update_Layout()
		...
		for i = 1, cnt-1 do
			for j = i+1 , cnt do
				if self.frame.icon[i].spell.remaining < self.frame.icon[j].spell.remaining then
	...
	function HDH_POWER_DK_TRACKER:CreateData(max)
		...
		local tmp = talent[self.name].tracker_enable;
		talent[self.name] = {}
		talent[self.name].tracker_enable = tmp;
