...
local function LoadDropDownButton(frame, idx, dataTable, onClickFunc)
...
function HDH_AddRow(rowFrame)
	...
	if name then
		if not HDH_DB_SaveSpell(key, CurSpec, no, id, name, always, glow, icon, isItem, CurTab) then
			print("|cffff0000AuraTracking:Error - |cffffff00'"..name.."("..key..")' 은(는) 이미 등록된 주문입니다.")
			return nil
		end
		if isItem then
...
function HDH_OnClickCopyAura(self)
	...
	if not name or not GetTabUnitInfo(CurTab) or name == GetTabUnitInfo(CurTab) or not spec or spec == 0 then 
...
function HDH_RefrashSettingUI(mode)
	...
	_G["SettingFrameSliderIcon"]:SetValue(icon.size)
	HDH_Adjust_Slider(_G["SettingFrameSliderIcon"], 20, 300)
...
function HDH_Adjust_Slider(self, min, max)
