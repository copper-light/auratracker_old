...
--------------------------------------------
	...
	function HDH_TRACKER:Init(name,unit)
		...
		if self.frame == nil then
			self.frame = CreateFrame("Frame", nil, HDHFrame)
		...
		self.frame:SetWidth(DB_OPTION.icon.size)
		self.frame:SetHeight(DB_OPTION.icon.size)
	end
	...
	function HDH_TRACKER:UpdateSetting()
		...
		if UI_LOCK then
			if self.frame.text then self.frame.text:SetPoint("BOTTOM", self.frame, "CENTER", 0,0) end
		end
		if not self.frame.icon then return end
	...
	function HDH_TRACKER:SetMove(move)
		...
		if move then
			if not self.frame.text then
				...
				text:ClearAllPoints()
				text:SetFont([[fonts\2002.ttf]], 12, "THICKOUTLINE")
				text:SetTextColor(1,0,0)
				text:SetWidth(150)
				text:SetHeight(70)
				text:SetPoint("BOTTOM", self.frame, "CENTER", 0,0)
...
local function PLAYER_ENTERING_WORLD()
	...
	HDH_AT_ADDON_Frame:RegisterEvent('PLAYER_TALENT_UPDATE')
...
HDH_AT_ADDON_Frame = CreateFrame("Frame") -- 애드온 최상위 프레임
