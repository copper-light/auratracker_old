...
do 
	...
	function CT_OnUpdateCooldown(self, elapsed)
		if not self:GetParent():GetParent().spell then self:Hide() return end
		...
		-- else
			...
			-- if not CT_HasTImer(self:GetParent():GetParent()) then
				-- self:Hide()
				-- self:GetParent():GetParent().timetext:SetText("");
				-- local t = {arg = self:GetParent():GetParent()}
				-- CT_Timer_Func(t)
			-- end
		end
	end 
	...
	function HDH_C_TRACKER:OnUpdateBarValue(elapsed)
		...
		if spell.remaining > HDH_C_TRACKER.EndCooldown and spell.duration > 0 then
			...
			if tracker.option.bar.enable and spell.duration > HDH_C_TRACKER.GlobalCooldown then
				...
				self:SetValue(tracker.option.bar.fill_bar and (maxV-spell.remaining) or (spell.remaining));
				-- self:SetValue(tracker.option.bar.fill_bar and GetTime() or spell.startTime + spell.remaining);
				tracker:moveSpark(tracker,self:GetParent(), spell);
			end
			if tracker.option.icon.hide_icon and not spell.isCharging and spell.duration > HDH_C_TRACKER.GlobalCooldown then
	...
	function CT_OnUpdateIcon(self) -- 거리 체크는 onUpdate 에서 처리해야함
		...
		if self.spell.curTime2 - (self.spell.delay2 or 0) < 0.15 then return end -- 10프레임
		...
		if self.spell.isCharging then --and self.spell.charges.duration > HDH_C_TRACKER.GlobalCooldown
			...
			-- print("CT_OnUpdateIcon",self.spell.charges.remaining)
	...
	function HDH_C_TRACKER:IsOk(id, name, isItem) -- 특성 스킬의 변경에 따른 스킬 표시 여부를 결정하기 위함
		...
		if HDH_AT_UTIL.IsTalentSpell(name) then
	...
	function HDH_C_TRACKER:InitIcons() -- HDH_TRACKER override
		...
		for i = 1 , #auraList do
			if self:IsOk(auraList[i].ID, auraList[i].Name, auraList[i].IsItem) then 
				...
				self.frame.pointer[auraList[i].Key or tostring(auraList[i].ID)] = f
				spell = {}
				...
				spell.slot = self:GetSlot(spell.id);
	...
	function HDH_C_TRACKER:UpdateSlot(idx)
		HDH_C_TRACKER.GET_SLOT = {}
		for i=1,120 do
			local type,id,subtype = GetActionInfo(i)
			if type then
				HDH_C_TRACKER.GET_SLOT[id] = i;
			end
		end
	end
	
	function HDH_C_TRACKER:GetSlot(id)
		if not HDH_C_TRACKER.GET_SLOT then
			self:UpdateSlot();
		end
		return HDH_C_TRACKER.GET_SLOT[id]
	end
...
------- HDH_C_TRACKER member function -----------	
	...
	function HDH_C_TRACKER:Update_Icon(f) -- f == f
		...
		if (spell.duration > 0) or (spell.charges.duration > 0) then -- 글로버 쿨다운 2초 포함
			if (option.icon.max_time == 0 and spell.always) or (option.icon.max_time > spell.remaining or spell.always) or (spell.duration < HDH_C_TRACKER.GlobalCooldown) then
				-- if not self.frame:IsShown() then self:ShowTracker(); end -- 비전투 상황일때 쿨다운 돈다고 하면, 화면 출력함.
				...
				if spell.isAble or spell.always then 
			...
			if spell.isAble or spell.always then if not f:IsShown() then f:Show() isUpdate= true end
			else if f:IsShown() then f:Hide() isUpdate= true end end
		end
	...
	function HDH_C_TRACKER:UpdateIconTexture(slot)
		self:UpdateSlot(slot);
		local id, slot_id;
		if self.frame.icon then
			for i= 1, #self.frame.icon do
				if self.frame.icon[i] then
					slot_id = select(2,GetActionInfo(slot));
					if ADJUST_ID[slot_id] == self.frame.icon[i].spell.id then
						self.frame.icon[i].spell.slot = self:GetSlot(slot_id);
					else
						self.frame.icon[i].spell.slot = self:GetSlot(self.frame.icon[i].spell.id);
					end
					if not self.frame.icon[i].spell.fix_icon and self.frame.icon[i].spell.slot then
						local texture = GetActionTexture(self.frame.icon[i].spell.slot);
						if (texture) then
							self.frame.icon[i].icon:SetTexture(texture);
						end
					else
						if self.frame.icon[i].spell.icon ~= self.frame.icon[i].icon:GetTexture() then
							self.frame.icon[i].icon:SetTexture(self.frame.icon[i].spell.icon);
						end
					end
				end
			end
