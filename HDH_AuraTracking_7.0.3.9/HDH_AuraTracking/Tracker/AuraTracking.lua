...
--------------------------------------------
	...
	function HDH_TRACKER:UpdateIconSettings(f)
		...
		if HDH_TRACKER[self.type] and self.option.icon.able_buff_cancel then
	...
	-- function HDH_TRACKER:UpdateValueText(text, value)
		-- if self.option.font.v1_abbreviate and then
	...
	function HDH_TRACKER:SetGlow(f, bool)
		...
		if bool and (f.spell and f.spell.glow) then
			if f.spell.glowCount 
				...
				self:ActionButton_ShowOverlayGlow(f) return
			elseif f.spell.glowV1 and f.spell.showValue and (f.spell.v1 >= f.spell.glowV1) then
				self:ActionButton_ShowOverlayGlow(f) return 
			end
		end
...
-- do
	...
	-- f:SetSize(100,20);
	-- f:SetPoint("CENTER");
	-- f:SetStatusBarTexture(HDH_TRACKER.BAR_TEXTURE[1].texture);
	-- f:SetMinMaxValues(1000000,1000003);
	-- f:SetValue(50)
	-- local v =  f:GetMinMaxValues();
