...
--------------------------------------------
		...
		if show then
			f:SetScript("OnEnter",function(frame) 
				...
				if not UI_LOCK and spell and spell.id then
					...
					if HDH_IS_UNIT[self.unit] and spell.index then
	...
	-- function HDH_TRACKER:UpdateBossGuid()
		-- self.boss_guid = {}
		-- for i = 1, MAX_BOSS_FRAMES do
			-- local unit = "boss"..i;
			-- if ExstisUnit(unit) and UnitGUID(unit) then
				-- self.boss_guid[i] = UnitGUID(unit);
			-- end
		-- end
	-- end
	...
	local function GetAurasAll(self) 
		...
		for i = 1, 40 do 
			...
			if self.option.base.tracking_boss_aura then
				if caster and (self:MatchBossUnit(caster) or (not UnitIsPlayer(caster) and UnitEffectiveLevel(caster) == -1)) then
				-- if i % 2 == 1 then
					-- print(name,ret)
...
do 
	...
	function HDH_AT_UTIL.AbbreviateValue(lengType, amount)
		if lengType == "kor" then
