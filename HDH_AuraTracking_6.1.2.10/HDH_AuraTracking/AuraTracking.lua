...
--------------------------------------------
	...
	function HDH_TRACKER:UpdateIconSettings(f)
		...
		counttext:SetTextHeight(op_font.fontsize)
	...
	local function GetAurasForResolveAndStagger(pointer, unit, filter, isbuff) 
		local curTime = GetTime()
		for i = 1, 40 do 
			name, _, _, count, _, duration, endTime, caster, _, _, id, canApplyAura, isBossDebuff, v1,v2,v3 = UnitAura(unit, i, filter)
			if not id then break end
			f = pointer[tostring(id)] or pointer[name]
			if f and f.spell then
				spell = f.spell
	...
	function HDH_TRACKER:Update()
		...
		if self.option.base.check_debuff then 
			self.GetAurasFunc(self.frame.pointer, self.unit, "HARMFUL"..(mine or ""), false)
		end
		
		if self.option.base.check_buff then 
			self.GetAurasFunc(self.frame.pointer, self.unit, "HELPFUL", true)
		end
	...
	function HDH_TRACKER:InitIcons()
		...
		for i = 1, #auraList do--[[
			...
			if ResolveID[spell.id] or StaggerID[spell.id] then
				isTanker = true
			end
		end
	...
	function HDH_TRACKER:SetGlow(f, bool)
		if bool and (f.spell and f.spell.glow) and f.spell.count >= (f.spell.glowCount or 0) then
