...
function HDH_GlowOptionFrame_ShowAni()
	if not GlowOptionFrame.ag then
		GlowOptionFrame.ag = GlowOptionFrame:CreateAnimationGroup()
		local ag = GlowOptionFrame.ag 
		ag.ap = ag:CreateAnimation("Alpha")
		ag.ap:SetOrder(1)
		ag.ap:SetDuration(0.3)
	...
	if not GlowOptionFrame:IsShown() then
		GlowOptionFrame.ag.ap:SetFromAlpha(0)
		GlowOptionFrame.ag.ap:SetToAlpha(1) 
		GlowOptionFrame.ag.tl:SetOffset(0,0)
		GlowOptionFrame.ag:SetScript("OnFinished",function()
			end) 
		GlowOptionFrame:Show()
		GlowOptionFrame.ag:Play()
	else
		GlowOptionFrame.ag:Stop()
		GlowOptionFrame:Show()
	end
	
end

function HDH_GlowOptionFrame_HideAni()
	if GlowOptionFrame.ag and GlowOptionFrame:IsShown() then
		GlowOptionFrame.ag.ap:SetFromAlpha(1)
		GlowOptionFrame.ag.ap:SetToAlpha(0)
		GlowOptionFrame.ag.tl:SetOffset(30,0)
		GlowOptionFrame.ag:SetScript("OnFinished",function()
				GlowOptionFrame:Hide()
			end) 
		GlowOptionFrame.ag:Play()
	end
end
...
function HDH_ClearRowData(rowFrame)
	...
	_G[rowFrame:GetName().."TextName"]:SetText("|cffaaaaaa(항목을 입력/수정하려면 행 클릭)   (이미지를 변경하려면 이미지 클릭)")
...
function HDH_ShowColorPicker(picker_type)
	...
	elseif picker_type == "cooldowntext" then
		ColorPickerFrame.func = function() end
		ColorPickerFrame.opacityFunc = HDH_OnSelectedColor
		ColorPickerFrame.cancelFunc = function() end
		ColorPickerFrame.hasOpacity = false
		ColorPickerFrame:SetColorRGB(unpack(option.font.textcolor))
	elseif picker_type == "cooldowntext5" then
		ColorPickerFrame.func = function() end
		ColorPickerFrame.opacityFunc = HDH_OnSelectedColor
		ColorPickerFrame.cancelFunc = function() end
		ColorPickerFrame.hasOpacity = false
		ColorPickerFrame:SetColorRGB(unpack(option.font.textcolor_5s))
	elseif picker_type == "counttext" then
		ColorPickerFrame.func = function() end
		ColorPickerFrame.opacityFunc = HDH_OnSelectedColor
		ColorPickerFrame.cancelFunc = function() end
		ColorPickerFrame.hasOpacity = false
		ColorPickerFrame:SetColorRGB(unpack(option.font.countcolor))
	else
...
function HDH_RefrashSettingUI(mode)
	...
	HDH_Adjust_Slider(_G[SetFrameContents:GetName().."SliderFont"], font.fontsize ,10, 100)
	...
	_G[SetFrameContents:GetName().."ButtonColorCooldownTextColor"]:SetTexture(unpack(font.textcolor))
	_G[SetFrameContents:GetName().."ButtonColorCooldownText5Color"]:SetTexture(unpack(font.textcolor_5s))
...
function HDH_OnShow_GlowFrame(self, check , id)
	if not check then HDH_GlowOptionFrame_HideAni() end
	local name = GetTabUnitInfo(CurTab)
	if not name then return end
	local db = DB_AURA.Talent[CurSpec][name]
	...
	if check == true then
		...
		GlowOptionFrame:SetParent(self)
		GlowOptionFrame:SetPoint("RIGHT",self,"LEFT",0,0)
		GlowOptionFrame.id = id
		GlowOptionFrameEB:SetText(db[tonumber(id)].GlowCount or 0)
		HDH_GlowOptionFrame_ShowAni()
	end
end

function HDH_OnClick_SaveGlowCount(count)
	local name = GetTabUnitInfo(CurTab)
	if not name then return end
	local db = DB_AURA.Talent[CurSpec][name]
...
function HDH_OnSelectedColor()
	...
	elseif t == 'cooldowntext' then
		option.font.textcolor = {ColorPickerFrame:GetColorRGB()}
		_G[SetFrameContents:GetName().."ButtonColorCooldownTextColor"]:SetTexture(unpack(option.font.textcolor))
	elseif t == 'cooldowntext5' then
	...
	elseif t == 'counttext' then
		option.font.countcolor = {ColorPickerFrame:GetColorRGB()}
		_G[SetFrameContents:GetName().."ButtonColorCountTextColor"]:SetTexture(unpack(option.font.countcolor))
	end
	if UI_LOCK then
		HDH_TRACKER.SetMoveAll(UI_LOCK)
	else
