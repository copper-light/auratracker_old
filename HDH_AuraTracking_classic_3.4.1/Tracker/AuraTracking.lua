...
HDH_TRACKER.ONUPDATE_FRAME_TERM = 0.02;
...
HDH_TRACKER.TYPE_BUFF = 1
HDH_TRACKER.TYPE_DEBUFF = 2

HDH_TRACKER.TYPE_GLOW_CONDITION_GT = 1
HDH_TRACKER.TYPE_GLOW_CONDITION_LT = 2
HDH_TRACKER.TYPE_GLOW_CONDITION_EQ = 3

-- local DDM_BAR_ICON_ORDER_LIST = {"목록 기준","남은시간 빠른순","남은시간 느린순","시전된 시간 빠른순","시전된 시간 느린순"};

HDH_GET_CLASS[HDH_TRACKER.TYPE_BUFF] = HDH_TRACKER; -- 클래스를 타입명에 맞춰 불러옴, cooldown 처럼 다른 클래스를 가져오기 위한 수단
HDH_GET_CLASS[HDH_TRACKER.TYPE_DEBUFF] = HDH_TRACKER;
HDH_GET_CLASS_NAME[HDH_TRACKER.TYPE_BUFF] = "HDH_TRACKER";
HDH_GET_CLASS_NAME[HDH_TRACKER.TYPE_DEBUFF] = "HDH_TRACKER";

HDH_TRACKER_LIST = { HDH_TRACKER.TYPE_BUFF, HDH_TRACKER.TYPE_DEBUFF }
HDH_TRACKER_LIST_L = {HDH_AT_L.BUFF, HDH_AT_L.DEBUFF}

HDH_UNIT_LIST_L = { 
					HDH_AT_L.PLAYER, HDH_AT_L.TARGET, HDH_AT_L.FOCUS, HDH_AT_L.PET, 
					HDH_AT_L.BOSS1, HDH_AT_L.BOSS2, HDH_AT_L.BOSS3, HDH_AT_L.BOSS4,HDH_AT_L.BOSS5, 
					HDH_AT_L.PARTY1, HDH_AT_L.PARTY2, HDH_AT_L.PARTY3, HDH_AT_L.PARTY4, 
					HDH_AT_L.ARENA1, HDH_AT_L.ARENA2, HDH_AT_L.ARENA3, HDH_AT_L.ARENA4, HDH_AT_L.ARENA5}
HDH_UNIT_LIST = { "player","target","focus","pet",
...
-- DB_VERSION = "0" -- DB 버전 : 버전이 다르면 DB를 초기화 시키기 위한 방안
-- DB_COMMON_CONFIG = {} -- 공유 설정
-- DB_CONFIG = {
-- 	{
-- 		["id"] = 1,
-- 		["name"] = "",
...
HDH_TRACKER.DefaultUIConfig = { 
							...
							icon   = { size = 40, margin_v = 4, margin_h = 4, 
										...
										cooldown = HDH_TRACKER.COOLDOWN_UP,
										line = 5, fix_location = false,
										order_by = HDH_TRACKER.ORDERBY_LIST
									}, 
							font   = {  show_cooldown = true, fontsize= 15, cd_location = HDH_TRACKER.FONT_LOCATION_TR, textcolor={1,1,0}, textcolor_5s={1,0,0}, time_type = HDH_TRACKER.TIME_TYPE_RAISE, -- 쿨 다운
...
HDH_TRACKER.DefaultTrackerConfig = { db_ver = HDH_AT_DB_VERSION,
							    -- unit = "player",
...
HDH_TRACKER.current_db_index = 1
...
function HDH_AT_DB_Release()
	HDH_AT_DB = nil
	HDH_AT_COMMON_DB = nil
end

function HDH_AT_DB_UI_Reset()
	HDH_AT_COMMON_DB = nil
	for i = 1, #HDH_AT_DB do
		db = HDH_AT_DB[i]
		db.ui = { 
			use_dont_common = false,
			location = { 
				x = math.floor(UIParent:GetWidth()/2), 
				y = math.floor(UIParent:GetHeight()/2)
			} 
		}
	end
end

function HDH_AT_GetDB()
	return HDH_AT_DB
end

function HDH_AT_GetDBIndex(idx)
	if not idx then 
		idx = HDH_TRACKER.current_db_index 
	end
	return HDH_AT_DB[idx]
end

function HDH_AT_GetTrackerDB(db_idx, tracker_idx)
	if not db_idx then 
		db_idx = HDH_TRACKER.current_db_index 
	end
	return HDH_AT_DB[db_idx].tracker_list[tracker_idx]
end

function HDH_AT_GetCommonDB()
	return HDH_AT_COMMON_DB
end

function HDH_AT_SwapDBTrackerIndex(idx1, idx2)
	local db = HDH_AT_GetDBIndex().tracker_list
	local tmp = db[idx1]
	db[idx1] = db[idx2]
	db[idx2] = tmp
end

function HDH_AT_IsExistsTrackerName(name)
	local ret = false
	local db = HDH_AT_GetDBIndex()

	for i = 1, #db.tracker_list do
		if name == db.tracker_list[i].name then
			ret = true
			break
		end
	end
...
function HDH_RefreshFrameLevel_All()
	tracker_list = HDH_AT_GetDBIndex().tracker_list
	local t
	for i= #tracker_list ,1 ,-1  do
		t = HDH_TRACKER.Get(i)
...
function HDH_DB_Add_Tracker(newName, newType, newUnit)
	...
	local new_tracker = { 	
											...
											unit = newUnit,
											spell_list = {},
											tracker = HDH_AT_UTIL.Deepcopy(HDH_TRACKER.DefaultTrackerConfig),
											ui = { 
												use_dont_common = false,
												location = { 
													x = math.floor(UIParent:GetWidth()/2), 
	...
	tracker_list[(#tracker_list + 1)] = new_tracker
	return new_tracker
end

function HDH_DB_Remove(idx)
	tracker_list = HDH_AT_GetDBIndex().tracker_list
	if idx > 0 then
		for i = idx , #tracker_list do -- 순서 정렬
			tracker_list[i] = tracker_list[i+1]
		end
	end
end

function HDH_DB_Modify(idx, newName, newType, newUnit)
	tracker_list = HDH_AT_GetDBIndex().tracker_list
	if tracker_list[idx] then
		tracker_list[idx].name = newName
		tracker_list[idx].type = newType
		tracker_list[idx].unit = newUnit
	end
end
...
-- local ICON_H_COORD = 0.93
local function UpdateCooldown(f, elapsed)
	local spell = f.spell;
	local tracker = f:GetParent().tracker;
	...
	if spell.remaining > 0.0 and spell.duration > 0 then
		...
		if tracker.db.ui.icon.cooldown ~= HDH_TRACKER.COOLDOWN_CIRCLE then
			if f.cd:GetObjectType() == "StatusBar" then f.cd:SetValue(spell.curTime) end
		end
		if tracker.db.ui.bar.enable and f.bar then
			local minV, maxV = f.bar:GetMinMaxValues();
			f.bar:SetValue(tracker.db.ui.bar.fill_bar and (maxV-spell.remaining) or (spell.remaining));
			if tracker.db.ui.bar.show_spark then
				tracker:moveSpark(f, spell);
			end
		else
			spell.per = math.ceil(spell.remaining / spell.duration * 100) / 100
			...
			if (f.iconSatCooldown.curSize ~= f.iconSatCooldown.preSize) then
				...
				if tracker.db.ui.icon.cooldown == HDH_TRACKER.COOLDOWN_LEFT then
					spell.texcoord = (0.07 + (math.ceil(0.86 * spell.per * 100) / 100))
					f.iconSatCooldown:SetWidth(f.iconSatCooldown.curSize)
					f.iconSatCooldown:SetTexCoord(0.07, spell.texcoord, 0.07, 0.93)
				elseif tracker.db.ui.icon.cooldown == HDH_TRACKER.COOLDOWN_RIGHT then
					spell.texcoord = (0.07 + 0.86 - (math.ceil(0.86 * spell.per * 100) / 100))
					f.iconSatCooldown:SetWidth(f.iconSatCooldown.curSize)
					f.iconSatCooldown:SetTexCoord(spell.texcoord, 0.93, 0.07, 0.93)
				elseif tracker.db.ui.icon.cooldown == HDH_TRACKER.COOLDOWN_UP then
					spell.texcoord = (0.07 + (math.ceil(0.86 * spell.per * 10000) / 10000))
...
function HDH_TRACKER:moveSpark(f, spell)
	if not self.db.ui.bar.show_spark then return end
	if self.db.ui.bar.fill_bar == self.db.ui.bar.reverse_progress then
...
local function AT_Timer_Func(self)
	if self and self.arg then
		local tracker = self.arg:GetParent() and self.arg:GetParent().tracker or nil;
		if tracker then
...
local function frameBaseSettings(f)
	...
	tempf:SetFrameLevel(f.iconframe:GetFrameLevel()+1)
...
--------------------------------------------
	...
	function HDH_TRACKER.new(db_tracker)
		local obj = nil;
		if HDH_GET_CLASS[db_tracker.type] then
			obj = {};
			setmetatable(obj, HDH_GET_CLASS[db_tracker.type])
			obj:Init(db_tracker)
	...
	function HDH_TRACKER.ClearAll()
		for idx = 1, #HDH_TRACKER.objList do
			HDH_TRACKER.RemoveList(idx)
		end
	end

	function HDH_TRACKER.RemoveList(idx) -- t = (string) or (tracker obj)
		t = HDH_TRACKER.Get(idx) -- 객체로 바꿔서
		if not t then return end
		HDH_DB_Remove(idx)
		t:Release()
		for i = idx , #HDH_TRACKER.objList do
			HDH_TRACKER.objList[i] = HDH_TRACKER.objList[i+1]
		end
	end

	function HDH_TRACKER.AddList(tracker)
		local newIdx = #HDH_TRACKER.objList + 1
		HDH_TRACKER.objList[newIdx] = tracker
	end

	function HDH_TRACKER.ModifyList(oldIdx, newIdx)
		if oldIdx == newIdx then return end
		HDH_TRACKER.objList[newIdx] = HDH_TRACKER.objList[oldIdx]
		HDH_TRACKER.objList[oldIdx] = nil
	end

	function HDH_TRACKER.Get(idx)
		return HDH_TRACKER.objList[idx] or nil
	end
	...
	function HDH_TRACKER.InitVaribles()
		-- 세팅 정보 저장하는 세이브인스턴스
		if not HDH_AT_COMMON_DB then
			HDH_AT_COMMON_DB = HDH_AT_UTIL.Deepcopy(HDH_TRACKER.DefaultUIConfig);
		else
			HDH_AT_COMMON_DB = HDH_AT_UTIL.CheckToUpdateDB(HDH_TRACKER.DefaultUIConfig, HDH_AT_COMMON_DB);
		end
		
		if not HDH_AT_DB or not GetNumTalentTabs() then
			HDH_AT_DB = {} 
		...
		local tracker
		for i = 1, #(db.tracker_list) do
			db_tracker = db.tracker_list[i]
			tracker = HDH_TRACKER.Get(i)
			if not tracker then
				HDH_TRACKER.new(db_tracker);
			else
				tracker:Init(db_tracker);
			end
		end
...
--------------------------------------------
	...
	function HDH_TRACKER:Init(db_tracker)
		self.db = db_tracker
		if self.db.tracker then
			self.db.tracker = HDH_AT_UTIL.CheckToUpdateDB(HDH_TRACKER.DefaultTrackerConfig, self.db.tracker);
		end
		if self.db.ui and self.db.ui.use_dont_common then
			self.db.ui.icon = HDH_AT_UTIL.CheckToUpdateDB(HDH_TRACKER.DefaultUIConfig.icon, self.db.ui.icon)
			self.db.ui.font = HDH_AT_UTIL.CheckToUpdateDB(HDH_TRACKER.DefaultUIConfig.font, self.db.ui.font)
			self.db.ui.bar = HDH_AT_UTIL.CheckToUpdateDB(HDH_TRACKER.DefaultUIConfig.bar, self.db.ui.bar)
		else
			common_db = HDH_AT_GetCommonDB()
			self.db.ui.icon = common_db.icon
			self.db.ui.font = common_db.font
			self.db.ui.bar = common_db.bar
		end

		if self.frame == nil then
			self.frame = CreateFrame("Frame", HDH_AT_ADDON_Frame:GetName()..self.db.name, HDH_AT_ADDON_Frame)
			self.frame:SetFrameStrata('MEDIUM')
			self.frame:SetClampedToScreen(true)
			self.frame.tracker = self
			self.frame.icon = {}
			self.frame.pointer = {}
		...
		self.frame:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT" , math.floor(self.db.ui.location.x), math.floor(self.db.ui.location.y))
		self.frame:SetSize(self.db.ui.icon.size, self.db.ui.icon.size)
		self:InitIcons()
	end
	...
	function HDH_TRACKER:IsHaveData()
		if self.db.tracker.tracking_all or self.db.tracker.tracking_boss_aura then
			cnt = HDH_TRACKER.MAX_ICONS_COUNT;
		else
			cnt = #self.db.spell_list;
		end
	...
	function HDH_TRACKER:ChangeFontLocation(f, fontf, location)
		parent = f.iconframe;
	...
	function HDH_TRACKER:UpdateIconSettings(f)
		local op_icon = self.db.ui.icon
		local op_font = self.db.ui.font
		local op_bar = self.db.ui.bar
		
		f:SetSize(op_icon.size,op_icon.size)
		f.iconframe:SetSize(op_icon.size,op_icon.size);
		self:SetGameTooltip(f, op_icon.show_spell_tooltip or false);
		-- self:SetGameTooltip(f, true);
		local icon = f.icon
		...
		self:ChangeCooldownType(f, op_icon.cooldown)
		
		if HDH_TRACKER.IsEqualClass(self.db.type, "HDH_TRACKER") and self.db.ui.icon.able_buff_cancel then
			f:SetMouseClickEnabled(true);
			f:RegisterForClicks("RightButtonUp");
			-- f:SetScript("OnClick",nil);
			f:SetScript("OnClick", function(self) 
				local tracker = self:GetParent().tracker;
				if tracker and tracker.db.unit and tracker.filter and self.spell.index then
					CancelUnitBuff(tracker.db.unit, self.spell.index); 
				end
			end);
		else
			f:SetMouseClickEnabled(false);
	...
	function HDH_TRACKER:UpdateArtBar(f)
		local op = self.db.ui.bar;
		local hide_icon = self.db.ui.icon.hide_icon;
		if op.enable then
			...
			self:SetGameTooltip(f.bar, HDH_AT_GetCommonDB().show_spell_tooltip or false);
		...
		if show then
			f:SetScript("OnEnter", function(f) 
				...
				if not UI_LOCK and spell and spell.id then
					...
					if HDH_TRACKER.IsEqualClass(self.db.type, "HDH_TRACKER") and spell.index then
						GameTooltip:SetUnitAura(self.db.unit, spell.index, self.filter);
					else	
	...
	local function OnMouseDown(self)
		local curT = self:GetParent().tracker
		if not curT then curT = self:GetParent():GetParent().tracker end
		...
		curT.frame:StartMoving()
	end
	
	local function OnMouseUp(self)
		local t = self:GetParent().tracker
		if not t then t = self:GetParent():GetParent().tracker end
		for k,v in pairs(HDH_TRACKER.GetList()) do
			if v.frame.text then
				v.frame.text:Show()
			end
		...
		if t then
			t.db.ui.location.x = math.floor(t.frame:GetLeft())
			t.db.ui.location.y = math.floor(t.frame:GetBottom())
			t.frame:ClearAllPoints()
			t.frame:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT" , t.db.ui.location.x , t.db.ui.location.y)
		end
	...
	local function OnDragUpdate(self)
		...
		t.db.ui.location.x = math.floor(t.frame:GetLeft())
		t.db.ui.location.y = math.floor(t.frame:GetBottom())
		t.frame.text:SetText(("%s\n|cffffffff(%d, %d)"):format(t.frame.text.text, t.db.ui.location.x, t.db.ui.location.y))
	end
	
	function HDH_TRACKER:CreateDummySpell(count)
		local icons =  self.frame.icon
		local option = self.db.ui
		...
		for i=1, count do
			...
			if self.db.type == L_BUFF then spell.isBuff = true
									  else spell.isBuff = false end
			if option.icon.cooldown == HDH_TRACKER.COOLDOWN_CIRCLE then
			...
			if option.bar.enable and f.bar then
	...
	function HDH_TRACKER:UpdateSetting()
		...
		for k,iconf in pairs(self.frame.icon) do
			...
			if not iconf.icon:IsDesaturated() then
				iconf.icon:SetAlpha(self.db.ui.icon.on_alpha)
				iconf.border:SetAlpha(self.db.ui.icon.on_alpha)
			else
				iconf.icon:SetAlpha(self.db.ui.icon.off_alpha)
				iconf.border:SetAlpha(self.db.ui.icon.off_alpha)
			end
		end	
		self.db.ui.location.x = self.frame:GetLeft()
		self.db.ui.location.y = self.frame:GetBottom()
	end
	...
	function HDH_TRACKER:ChangeCooldownType(f, cooldown_type)
		if cooldown_type == HDH_TRACKER.COOLDOWN_UP then 
			...
			f.iconSatCooldown:SetHeight(self.db.ui.icon.size)
			f.iconSatCooldown.spark:SetSize(self.db.ui.icon.size*1.2, 8);
			f.iconSatCooldown.spark:SetTexture("Interface\\AddOns\\HDH_AuraTracking\\Texture\\UI-CastingBar-Spark_v");
		...
		elseif cooldown_type == HDH_TRACKER.COOLDOWN_LEFT  then 
			...
			f.iconSatCooldown:SetPoint("TOPLEFT", f.iconframe,"TOPLEFT",0,0)
			f.iconSatCooldown:SetPoint("BOTTOMLEFT", f.iconframe,"BOTTOMLEFT",0,0)
			f.iconSatCooldown:SetWidth(self.db.ui.icon.size)

			f.iconSatCooldown.spark:SetSize(8, self.db.ui.icon.size*1.1);
			f.iconSatCooldown.spark:SetTexture("Interface\\AddOns\\HDH_AuraTracking\\Texture\\UI-CastingBar-Spark");
	...
	function HDH_TRACKER:UpdateTimeText(text, value)
		if self.db.ui.font.time_type == HDH_TRACKER.TIME_TYPE_RAISE then value = value + 1; end
		if value > 5 then text:SetTextColor(unpack(self.db.ui.font.textcolor)) 
				     else text:SetTextColor(unpack(self.db.ui.font.textcolor_5s)) end
		if value <= 9.9 and self.db.ui.font.time_type == HDH_TRACKER.TIME_TYPE_FLOAT then 
	...
	function HDH_TRACKER:UpdateBarValue(f, isEnding)
		if f.bar and f.name then
			if self.db.ui.bar.fill_bar then
				if isEnding then
					f.bar:SetMinMaxValues(0,1); 
					f.bar:SetValue(1); 
					f.name:SetTextColor(unpack(self.db.ui.bar.name_color_off));
					-- if not f.spell.is_buff and self.db.ui.icon.default_color and HDH_IS_UNIT[self.db.unit] then
					if (f.spell.is_buff and f.spell.dispelType ) and self.db.ui.icon.default_color and HDH_IS_UNIT[self.db.unit] then
					...
					elseif self.db.ui.bar.use_full_color then
						f.bar:SetStatusBarColor(unpack(self.db.ui.bar.full_color));
					end
					f.bar.spark:Hide();
				else
					if (f.spell.is_buff and f.spell.dispelType ) and self.db.ui.icon.default_color and HDH_IS_UNIT[self.db.unit] then
						...
						f.bar:SetStatusBarColor(unpack(self.db.ui.bar.color));
					end
					local maxV = f.spell.endTime - f.spell.startTime;
					f.bar:SetMinMaxValues(0, maxV); 
					f.bar:SetValue(maxV-f.spell.remaining); 
					f.name:SetTextColor(unpack(self.db.ui.bar.name_color));
					if self.db.ui.bar.show_spark and f.spell.duration > 0 then f.bar.spark:Show(); 
	...
	-- 	if self.db.ui.base.list_share then
	-- 		return self.db.ui.base.share_spec
	-- 	else
	...
	function HDH_TRACKER:LoadOrderFunc()
		if self.db.ui.icon.order_by == HDH_TRACKER.ORDERBY_LIST then
			self.OrderFunc = nil;
		elseif self.db.ui.icon.order_by == HDH_TRACKER.ORDERBY_CD_ASC then
			self.OrderFunc = self.InAsendingOrderByTime
		elseif self.db.ui.icon.order_by == HDH_TRACKER.ORDERBY_CD_DESC then
			self.OrderFunc = self.InDesendingOrderByTime
		elseif self.db.ui.icon.order_by == HDH_TRACKER.ORDERBY_CAST_ASC then
			self.OrderFunc = self.InAsendingOrderByCast;
		elseif self.db.ui.icon.order_by == HDH_TRACKER.ORDERBY_CAST_DESC then
	...
	function HDH_TRACKER:UpdateIcons()
		...
		local line = self.db.ui.icon.line or 10-- 한줄에 몇개의 아이콘 표시
		local margin_h = self.db.ui.icon.margin_h
		local margin_v = self.db.ui.icon.margin_v
		local size = self.db.ui.icon.size -- 아이콘 간격 띄우는 기본값
		local revers_v = self.db.ui.icon.revers_v -- 상하반전
		local revers_h = self.db.ui.icon.revers_h -- 좌우반전
		local icons = self.frame.icon
		...
		for k,f in ipairs(icons) do
			...
			if f.spell.isUpdate then
				...
				if self.db.tracker.check_only_mine then
				...
				if f.spell.duration == 0 then 
					...
					f.icon:SetAlpha(self.db.ui.icon.on_alpha)
					f.border:SetAlpha(self.db.ui.icon.on_alpha)
					...
					f.icon:SetAlpha(self.db.ui.icon.off_alpha)
					f.border:SetAlpha(self.db.ui.icon.off_alpha)
					
					-- f.iconSatCooldown:SetSize(math.min(f.iconSatCooldown:GetWidth(), f.iconframe:GetWidth()), 
					-- 							math.min(f.iconSatCooldown:GetHeight(), f.iconframe:GetHeight()))
					f.iconSatCooldown:SetAlpha(self.db.ui.icon.on_alpha)
					f.iconSatCooldown.spark:Show()
				end
			    
				if f.spell.is_buff then 
					-- f.border:SetVertexColor(unpack(self.db.ui.icon.buff_color)) 
					if f.spell.dispelType and self.db.ui.icon.default_color then 
						f.border:SetVertexColor(DebuffTypeColor[f.spell.dispelType or ""].r,
					   							DebuffTypeColor[f.spell.dispelType or ""].g,
												DebuffTypeColor[f.spell.dispelType or ""].b)
					else 
						f.border:SetVertexColor(unpack(self.db.ui.icon.buff_color)) 
					...
					if self.db.ui.icon.default_color then 
						f.border:SetVertexColor(DebuffTypeColor[f.spell.dispelType or ""].r,
					   							DebuffTypeColor[f.spell.dispelType or ""].g,
												DebuffTypeColor[f.spell.dispelType or ""].b)
					else 
						f.border:SetVertexColor(unpack(self.db.ui.icon.debuff_color)) 
					end 
				end
				if self.db.ui.icon.cooldown == HDH_TRACKER.COOLDOWN_CIRCLE then
				...
				if self.db.ui.bar.enable and f.bar then
					-- f.bar:SetMinMaxValues(f.spell.startTime, f.spell.endTime);
	...
	local function GetUnitAuras(unit, idx, filter)
		name, icon, count, dispelType, duration, endTime, caster, _, _, id, canApplyAura, isBossDebuff = UnitAura(unit, idx, filter)
		if endTime == 0 then
			-- libDuration, libEndTime = LibClassicDurations:GetAuraDurationByUnit(unit, id)	
		...
		return name, icon, count, dispelType, duration, endTime, caster, _, _, id, canApplyAura, isBossDebuff
	end
	...
	local function GetAuras(self)
		...
		local f
		for i = 1, 40 do 
			name, icon, count, dispelType, duration, endTime, caster, _, _, id, _, isBossDebuff, _,_, v1, v2, v3 = GetUnitAuras(self.db.unit, i, self.filter)
	...
	local function GetAurasAll(self) 
		...
		local f
		for i = 1, 40 do 
			name, icon, count, dispelType, duration, endTime, caster, _, _, id, canApplyAura, isBossDebuff = GetUnitAuras(self.db.unit, i, self.filter)
			if not id then break end
			-- GameTooltip:SetUnitAura(self.db.unit, i, self.filter);
			-- f = self.frame.pointer[name];
			-- if not f then 
				-- self.frame.pointer[id] = f;
			-- end
			if self.db.tracker.tracking_boss_aura then
			...
			if f then
				...
				spell.startTime = endTime - duration
				spell.icon = icon
				spell.is_buff = (self.db.type == HDH_TRACKER.TYPE_BUFF);
				spell.index = i; -- 툴팁을 위해, 순서
	...
	function HDH_TRACKER:Update()
		...
		if not UnitExists(self.db.unit) or not self.frame.pointer or not self.db then 
			self.frame:Hide() return 
		end
		
		self.GetAurasFunc(self);
		if (self:UpdateIcons(self) > 0) or self.db.ui.icon.always_show or UnitAffectingCombat("player") then
			-- self.frame:Show()
			self:ShowTracker();
		else
			-- self.frame:Hide()
	...
	function HDH_TRACKER:InitIcons()
		...
		if not self.db then return end 	-- 설정 정보 없으면 패스
		if not self.db.spell_list then return end 				-- 특성 정보 없으면 패스
		local auraList = self.db.spell_list or {}
		local name, icon, spellID
		local spell 
		local f
		local iconIdx = 0;
		local isBuff = (self.db.type == HDH_TRACKER.TYPE_BUFF);
		if self.db.tracker.tracking_all or self.db.tracker.tracking_boss_aura then
			...
			for i = 1, #auraList do
				if not self:IsIgnoreSpellByTalentSpell(auraList[i]) then
					...
					spell.glow_list = auraList[i].Glow
					...
					f.icon:SetDesaturated(1)
					f.iconSatCooldown:SetTexture(auraList[i].Texture or "Interface\\ICONS\\INV_Misc_QuestionMark")
					f.iconSatCooldown:SetDesaturated(nil)
					-- f.iconSatCooldown:SetHeight(f.icon:GetHeight() / 2)
					-- f.icon:SetDesaturated(1)
					-- f.icon:SetAlpha(self.option.icon.off_alpha)
					-- f.border:SetAlpha(self.option.icon.off_alpha)
					self:ChangeCooldownType(f, self.db.ui.icon.cooldown)
					self:SetGlow(f, false)
		...
		if self.db.tracker.check_only_mine then  filter = filter.."|PLAYER" end
		self.filter = filter;
		...
		if #(self.frame.icon) > 0 or self.db.tracker.tracking_all or self.db.tracker.tracking_boss_aura then
			self.frame:RegisterEvent('UNIT_AURA')
			if self.db.unit == 'target' then
				self.frame:RegisterEvent('PLAYER_TARGET_CHANGED')
			elseif self.db.unit == 'focus' then
				self.frame:RegisterEvent('PLAYER_FOCUS_CHANGED')
			elseif string.find(self.db.unit, "boss") then 
				self.frame:RegisterEvent('INSTANCE_ENCOUNTER_ENGAGE_UNIT')
			elseif string.find(self.db.unit, "party") then
				self.frame:RegisterEvent('GROUP_ROSTER_UPDATE')
			elseif self.db.unit == 'pet' then
				self.frame:RegisterEvent('UNIT_PET')
			elseif string.find(self.db.unit, 'arena') then
	...
	function HDH_TRACKER:SetGlow(f, bool)
		...
		if bool and f.spell.glow_list and #f.spell.glow_list > 0 then
			for idx = 1, #(f.spell.glow_list) do
				item = f.spell.glow_list[idx]
				if item.enable then
					v = math.ceil(f.spell[item.type])
					if item.type == "count" and v == 0 then
						v = 1
					end
					if item.condition == HDH_TRACKER.TYPE_GLOW_CONDITION_GT then 
						if v >= item.value then
							self:ActionButton_ShowOverlayGlow(f)
							return
						end
					elseif item.condition == HDH_TRACKER.TYPE_GLOW_CONDITION_LT then 
						if v <= item.value then
							self:ActionButton_ShowOverlayGlow(f)
							return
						end
					elseif item.condition == HDH_TRACKER.TYPE_GLOW_CONDITION_EQ then
						if v == item.value then
							self:ActionButton_ShowOverlayGlow(f)
							return
						end
...
local function PLAYER_ENTERING_WORLD()
	if not HDH_TRACKER.IsLoaded then 
		print('|cffffff00HDH - AuraTracking |cffffffff(Config: /at, /auratracking, /ㅁㅅ)')
    ...
    -- LibClassicDurations:RegisterFrame(HDH_AT_ADDON_Frame)

	HDH_TRACKER.InitVaribles()
	
	for k, tracker in ipairs(HDH_TRACKER.objList) do
...
function OnEventTracker(self, event, ...)
	if event == 'UNIT_AURA' then
		local unit = select(1,...)
		if self.tracker and unit == self.tracker.db.unit then
			HDH_UNIT_AURA(self.tracker)
		end
	...
	elseif event == 'UNIT_PET' then
		self.tracker:RunTimer("UNIT_PET", 0.5, HDH_UNIT_AURA, self.tracker) 
	elseif event == 'ARENA_OPPONENT_UPDATE' then
		self.tracker:RunTimer("ARENA_OPPONENT_UPDATE", 0.5, HDH_UNIT_AURA, self.tracker) 
	elseif event == "ENCOUNTER_START" then
		self.tracker.isRaiding = true;
	elseif event == "ENCOUNTER_END" then
		self.tracker.isRaiding = false;
	end
end
...
local function OnEvent(self, event, ...)
	if event =='ACTIVE_TALENT_GROUP_CHANGED' then
		...
		for k, tracker in pairs(HDH_TRACKER.GetList()) do
			tracker:ACTIVE_TALENT_GROUP_CHANGED()
		end
...
local function HDH_AT_OnLoad(self)
...
HDH_AT_OnLoad(HDH_AT_ADDON_Frame)
