...
function HDH_InitVaribles()
	...
	if not DB_OPTION or not DB_OPTION.HDH_AT_SETTING_VERSION or (tonumber(DB_OPTION.HDH_AT_SETTING_VERSION) < HDH_AT_SETTING_VERSION) then
		...
		DB_OPTION = { always_show = false, 
						 ...
						 focus  = { x = center_x, y = center_y+100, revers_h = false, revers_v = false,
									cooldown = 1, -- 1위로, 2아래로 3왼쪽으로 4오른쪽으로 5 원형
									line = 10  }, -- 디버프 아이콘 위치 
						 icon   = { size = 30, margin = 4, on_alpha = 1, off_alpha = 0.5, show_cooldown = true },   -- 아이콘 크기
	...
	if not DB_OPTION["player"].cooldown then 
		DB_OPTION["player"].cooldown = 1
		DB_OPTION["target"].cooldown = 1
	end
	
	if not DB_OPTION["focus"].cooldown then
		DB_OPTION["focus"].cooldown = 1
		DB_OPTION["focus"].line = 10
	end
	
	if not DB_OPTION.icon.show_cooldown then
...
function frameBaseSettings(f)
	...
	f.cooldown1:SetStatusBarTexture(0,0,0,0.75)
	...
	f.cooldown2:SetSwipeColor(0,0,0,0.75)
...
function HDH_CreateDummyIcon(unit, count)
	...
	for i=1, count do
		...
		if cooldown_type == COOLDOWN_CIRCLE then
				f.cooldown:SetCooldown(spell.startTime,spell.duration)
			else
...
function HDH_ChangeCooldownType(f, cooldown_type)
	if cooldown_type == COOLDOWN_UP then 
		f.cooldown = f.cooldown1
		f.cooldown:SetOrientation("Vertical")
		f.cooldown:SetReverseFill(false)
	...
	elseif cooldown_type == COOLDOWN_DOWN  then 
		f.cooldown = f.cooldown1
		f.cooldown:SetOrientation("Vertical")
		f.cooldown:SetReverseFill(true)
		if f.cooldown2:IsShown() then
			f.cooldown2:Hide()
		end
		f.counttext:SetFont(DB_OPTION.font.style, DB_OPTION.font.fontsize, "OUTLINE")
	elseif cooldown_type == COOLDOWN_LEFT  then 
		f.cooldown = f.cooldown1
		f.cooldown:SetOrientation("Horizontal"); 
		f.cooldown:SetReverseFill(true)
		if f.cooldown2:IsShown() then
			f.cooldown2:Hide()
		end
...
function HDH_UpdateAuraICONS(unit)
	...
	for k,f in ipairs(aura) do
		...
		if f.spell.isUpdate then
			...
			if cooldown_type == COOLDOWN_CIRCLE then
				f.cooldown:SetCooldown(f.spell.startTime, f.spell.duration)
			else
...
local cashFrameByName = {}
function HDH_GetFrameByName(unit, name)
	if ICONS[unit] then
		for i = 1, #ICONS[unit] do
			if ICONS[unit][i].spell and ICONS[unit][i].spell.name == name then
			return ICONS[unit][i] 
			end
		end
	end
...
function HDH_UpdateAura(unit)
	...
	elseif unit == 'focus'  then filter = "HARMFUL | PLAYER"
					        else return 0 end
	...
	for i = 1, 40 do 
		...
		f = AuraPointer[unit][tostring(id)] or AuraPointer[unit][name]
