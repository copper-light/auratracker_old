...
HDH_TRACKER_LIST[#HDH_TRACKER_LIST+1] = "스킬쿨다운:player" -- 유닛은 명확하게는 추적 타입으로 보는게 맞지만 at 에서 이미 그렇게 사용하기 때문에 그냥 유닛 리스트로 넣어서 사용함
HDH_GET_CLASS["스킬쿨다운:player"] = HDH_C_TRACKER -- at 의 new 함수에서 cooldown 의 클래스를 불러오는게 가능하도록 클래스 리스트에 추가해둠
...
HDH_OLD_TRACKER["Player_cooldown"] = "스킬쿨다운:player";
HDH_OLD_TRACKER["Pet_cooldown"] = PET_TYPE;
HDH_OLD_TRACKER["cooldown"] = "스킬쿨다운:player";
...
function CT_OnUpdateIcon(self) -- 거리 체크는 onUpdate 에서 처리해야함
	...
	if self.spell.isCharging and self.spell.duration > HDH_C_TRACKER.GlobalCooldown then
	...
	if self:GetParent().parent:GetSlot(self.spell.id) then
		self.spell.newRange = IsActionInRange(self:GetParent().parent:GetSlot(self.spell.id)) 
	else
...
do 
	...
	function HDH_C_TRACKER:Update() -- HDH_TRACKER override
		...
		if not UI_LOCK and self.unit == PET_TYPE and not UnitExists("pet") then
	...
	function HDH_C_TRACKER:InitIcons() -- HDH_TRACKER override
		...
		if #(self.frame.icon) > 0 and self.unit == PET_TYPE then
...
------- HDH_C_TRACKER member function -----------	
	...
	function HDH_C_TRACKER:Update_Usable(f)
		...
		local slot = self:GetSlot(spell.id);
		if slot then
			spell.inRange = IsActionInRange(slot); -- 1:true,0:false,nil:non target
		else
